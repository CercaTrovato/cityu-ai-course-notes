...
> **原始材料**：`Module 3 - Fairness Bias and Discrimination in AI.pdf`（89 页） ｜ **转录**：`pending`（本讲录音尚未导出，🎙️ 各格暂写"待转录补充"；考点可信度 🟡 封顶）
...
| 监督学习 · 标注数据 · 准确率 / 精确率 / 召回率 · 过拟合 | L0 入场基线，本讲 §2.12.4 的公平性指标就建立在"真阳性率 / 假阳性率"这两个词上 | [[AI_Ethics_and_Regulations/_meta/知识层级台账\|知识层级台账]] L0 |
...
待转录补充（⚠️ p.32 的超链接很可能是课上播放的视频，转录到位后核对）。
...
待转录补充（⚠️ p.42 的案例讲义只有骨架，细节很可能在课上口述；转录到位后补）。
...
待转录补充（⚠️ p.56 的两个问题很可能是课堂讨论题；转录到位后补上同学与教授的回答）。
...
本库里有一个现成的抽样偏见案例：IS6400 用的 Airbnb 数据集只覆盖美国 6 个城市，且纽约 + 洛杉矶占 75%（[[Business_Data_Analytics/_meta/数据集卡片|数据集卡片]] §5.6）——用它训练的定价模型，拿到其他城市就是 p.58 说的"只调查一个街区"。
...
待转录补充（⚠️ p.62 的"照镜子还是改善"是讨论题，转录到位后补课堂回答）。
...
| 级别 | 含义 | 本讲 |
|---|---|---|
| 🔴 教授明示 | 转录里教授明确说过会考 / 要记 —— 引用原话 | **本讲暂无**（转录未导出，🟡 封顶） |
...
**总页数：89 页。✅ 88 页在 §2 有实质讲解；唯一例外 p.1（封面）。** 本讲无转录，「课堂覆盖」整列写 `—`。

| 笔记小节 | 讲义页 | 内容 | 课堂覆盖 |
|---|---|---|---|
| —（封面） | p.1 | 标题 + 讲师 | — |
| §2.1 | p.2 | 议程三行（页脚 "1-2"） | — |
| §2.1 | p.3–5 | 导论 · 四个底层想法 · 什么是正义 | — |
| §2.2 | p.6 | 历史基石四家 | — |
| §2.4.1 | p.7 | 正义即公平：两条原则 + 优先次序 | — |
| §2.4.1 | p.8 | 平等自由原则 | — |
| §2.4.1 | p.9 | 差异原则 | — |
| §2.4.1 | p.10 | 公平机会平等原则 | — |
| §2.4.2 | p.11 | 原初状态 · 无知之幕 · 最大最小规则 | — |
| §2.4.3 | p.12 | 罗尔斯方法 | — |
| §2.4.3 | p.13 | 公平原则的判据 | — |
| §2.4.2 | p.14 | 原初状态与无知之幕的定义 | — |
| §2.4.3 | p.15 | 与康德要求一致（三检验 + 罗尔斯的主张） | — |
| §2.1 | p.16 | **纯图片页**：Equality vs Equity（已视觉复核） | — |
| §2.3 | p.17 | 分配正义的主要理论（四种） | — |
| §2.2 | p.18 | 分配 / 报应 / 补偿 + 程序 / 互动 | — |
| §2.3 | p.19 | 分配正义的五种理论 | — |
| §2.3 | p.20 | 形式原则 + 销售员例子 + 实质原则 | — |
| §2.5 | p.21 | 哲学偏见四条 | — |
| §2.6 | p.22 | 心理学视角导入 | — |
| §2.6 | p.23 | 心理学里的公平定义 | — |
| §2.6 | p.24 | 三种公平启发式 | — |
| §2.7.1 | p.25 | 认知偏差定义 + Tversky 引言 + 五种一句话 | — |
| §2.7.2 | p.26 | **纯图片页**：自利偏差卡片（已视觉复核、誊录） | — |
| §2.7.2 | p.27 | **纯图片页**：确认偏差卡片（已视觉复核、誊录） | — |
| §2.7.2 | p.28 | **近纯图片页**：损失厌恶卡片 + 前景理论价值函数图（已视觉复核、誊录、读图） | — |
| §2.7.2 | p.29 | **纯图片页**：锚定偏差卡片（已视觉复核、誊录） | — |
| §2.7.2 | p.30 | **纯图片页**：框架效应卡片（已视觉复核、誊录） | — |
| §2.8.1 | p.31 | 情绪与公平四条 | — |
| §2.8.2 | p.32 | 分配公平实验（含超链接） | — |
| §2.8.3 | p.33 | 程序公平效应四要素 | — |
| §2.8.3 | p.34 | 互动公平（信息 / 人际）+ 领导行为 + 团队 | — |
| §2.8.4 | p.35 | 公平感知的四种测量 | — |
| §2.9.1 | p.36 | 商业视角导入 | — |
| §2.9.1 | p.37 | 组织公正四维度 | — |
| §2.9.1 | p.38 | 对员工行为的四类影响 | — |
| §2.9.2 | p.39 | 招聘与晋升中的偏见 | — |
| §2.9.2 | p.40 | 公平的薪酬体系 | — |
| §2.9.2 | p.41 | 设计公平的流程 | — |
| §2.9.3 | p.42 | 案例：推行程序公正（骨架） | — |
| §2.9.3 | p.43 | 整合三视角 | — |
| §2.9.3 | p.44 | 第一部分小结 | — |
| §2.10.1 | p.45 | 章节扉页 "AI BIASES"（仅标题，已在正文点明） | — |
| §2.10.1 | p.46 | 第二部分学习目标（本节对应学习目标已写入 §2.10.1） | — |
| §2.10.1 | p.47 | 为什么管理者要在乎（三类风险） | — |
| §2.10.2 | p.48 | AI 偏见定义（IBM） | — |
| §2.10.3 | p.49 | AI 复习 | — |
| §2.10.3 | p.50 | AI 八个核心概念 | — |
| §2.10.3 | p.51 | 商业里的四类 AI 系统 | — |
| §2.10.4 | p.52 | 偏见 vs 公平 + 张力 | — |
| §2.11.1 | p.53 | 偏见的定义与重要性（四格；页脚 "Sample Footer Text"） | — |
| §2.11.3 | p.54 | 五种偏见类型（页脚 "Sample Footer Text"） | — |
| §2.11.1 | p.55 | 科技史三先例 + "AI 能规模化歧视" | — |
| §2.11.2 | p.56 | 亚马逊案 + 两个讨论题 | — |
| §2.11.4 | p.57 | 数据偏见 | — |
| §2.11.4 | p.58 | 抽样偏见 | — |
| §2.11.4 | p.59 | 标签偏见 | — |
| §2.11.4 | p.60 | 测量偏见（代理变量） | — |
| §2.11.5 | p.61 | 算法偏见（三格图） | — |
| §2.11.5 | p.62 | 社会偏见 + "照镜子还是改善" | — |
| §2.11.5 | p.63 | 反馈回路 | — |
| §2.11.6 | p.64 | 训练 vs 部署：生命周期 | — |
| §2.11.6 | p.65 | 第二部分小结：谁受益 / 谁受害 / 谁缺席 | — |
| §2.12 | p.66 | 章节扉页 "Mitigation Strategies & Leadership Responsibility"（仅标题，已在正文点明） | — |
| §2.12.1 | p.67 | 群体公平 vs 个体公平 | — |
| §2.12.2 | p.68 | 反事实公平 | — |
| §2.12.3 | p.69 | 差别性影响分析 | — |
| §2.12.4 | p.70 | 三个公平性指标 + 五格截图（已视觉复核、读图） | — |
| §2.12.5 | p.71 | 准确率与公平的权衡 | — |
| §2.13.1 | p.72 | 多样化与代表性数据 | — |
| §2.13.1 | p.73 | 偏见审计与测试（Facebook） | — |
| §2.13.2 | p.74 | 可解释 AI（SHAP / LIME / 特征重要性） | — |
| §2.13.2 | p.75 | 人在环路 | — |
| §2.13.3 | p.76 | 包容性设计 | — |
| §2.13.3 | p.77 | 透明与文档 + 模型卡模板图（已视觉复核、誊录九栏） | — |
| §2.14.1 | p.78 | 偏见对企业的四类影响（页脚 "Sample Footer Text"） | — |
| §2.14.1 | p.79 | 监管环境与合规（EU AI Act）（页脚 "Sample Footer Text"） | — |
| §2.14.2 | p.80 | 伦理 AI 治理模型三条 | — |
| §2.14.2 | p.81 | 治理六步弧线图（与 M01 p.56 相同） | — |
| §2.14.3 | p.82 | 多元 AI 团队 | — |
| §2.14.3 | p.83 | 利益相关者参与 | — |
| §2.14.3 | p.84 | AI 伦理委员会（Salesforce） | — |
| §2.15 | p.85 | 你会成为什么样的领导者 | — |
| §2.15 | p.86 | 你在塑造公平 AI 中的角色 | — |
| §2.15 | p.87 | 关键概念回顾 | — |
| §2.15 | p.88 | 结语 | — |
| §2.15 | p.89 | 结束语（整页一句） | — |
...
本讲无转录，无法判断。转录到位后按 [[转录处理规则]] 回填此节与 §8「课堂覆盖」列。

### 9.2 课上讲了但课件没有

本讲无转录，无法比对。⚠️ 以下几处**很可能**有课堂口述内容，转录到位后优先核对：p.32 的超链接（疑为视频）、p.42 的案例（讲义只有骨架）、p.56 / p.62 / p.71 / p.85 的讨论题。
...
| **跨课链接** | IS5542 [[M01-导论-生成式AI基础与企业落地]] §2.9.7 "偏见的五个入口"与本讲 §2.11 互证；§2.7.2 / §2.9.3 对 XAI 与人在环路的立场更硬；IS6400 [[Business_Data_Analytics/_meta/数据集卡片\|数据集卡片]] §5.6 是抽样偏见的现成例子；M02 §2.7.8 分配公正是本讲的哲学原型 | 本库 |
...
| ② | **转录** | 本讲录音尚未导出到 `transcripts/`。到位后：回填 39 处"待转录补充"、§8 课堂覆盖列、§9.1–9.2，并把 §6.2 里教授明确强调的条目升为 🔴 |
...
1. **最薄弱的一节**：§2.8（心理学视角 III）与 §2.9（商业视角）。讲义 p.31–43 每页只有四个名词，几乎没有定义和例子；本笔记的解释大部分是 💡 笔记补充，**教授课上怎么讲这些页、强调了什么，一无所知**。转录到位前，这两节的"为什么需要它"都是我的推断。
...
3. **"因为材料没有所以推断"的判断**：① p.5–20 出自 Velasquez 教科书——若错，只影响补充读物的推荐；② p.70 截图来自 IBM AIF360、"公平区间 ±0.1 / 0.8–1.25"取自该演示——若错，§4.5 的"公平区间"列不可信，但指标定义不受影响；③ 五分之四规则、不可能性定理等为讲义没有的外部内容——若考试只认讲义，这些不会加分但也不会扣分；④ 把 p.17 与 p.19 合并、把 p.54 与 p.57–63 对照，是我的结构判断——若教授把它们当两套独立知识点考，读者需要按 §4.4 分开复述；⑤ 讲义最终"站在改善一边"（§2.15）是我对 p.88–89 的解读，教授本人的立场要等转录。
...
- 上一讲 [[M02-道德理论及其应用]] ｜ 课程入口 [[AI_Ethics_and_Regulations/00-课程总览|00-课程总览]]
- [[AI_Ethics_and_Regulations/_meta/知识层级台账|知识层级台账]] · [[AI_Ethics_and_Regulations/_meta/术语表|术语表]] · [[AI_Ethics_and_Regulations/_meta/考点库|考点库]] · [[AI_Ethics_and_Regulations/_meta/作业与DDL|作业与DDL]]
