...
✅ **Assignment 1 已于 2026-09-22 提交**。个人作业成品不进入公开仓库；课程任务信息见 [[IS5113_AI_Ethics_and_Regulations/_meta/作业与DDL#⏰ 最近的截止|作业与DDL]]，讲评后仅回填可公开的课程知识点。

✅ **M04 转录（本地 Whisper，`00:00 → 02:11:34`，969 段，完整）2026-09-23 已合并 → [[M04-AI透明-从看得见到信得过]] v1.0**：33 个原有格落态（A 16 / B 3 / C 13 / 存疑 1），**另新增 8 个 🎙️ 纯课堂小节**。★ **这一讲讲的和讲义差得最远**——教授把讲义 p.25–43 的十九页企业案例整段留给学生自读（`02:07:45`），改用四个讲义没有的真实案例：**Amazon 招聘 AI 筛选系统 → COMPAS 再犯风险评分 → Apple Card 信用额度 → Air Canada 聊天机器人**（共约 43 分钟），并在 `02:03:36` 给了一张四案对比的"summary"表，串成 AI 透明的十年演进（数据 → 过程 → 个体解释 → 内容可信）。§6 新增 7 条 🔴（含 2 条反向降权）。⚠️ 讲义 p.17 的论文二（临床医生实验）全文检索不到对应段落，标**存疑**不标 ⏭️、不可降权。
...
| 4 | Transparency and Explainability | [[M04-AI透明-从看得见到信得过]] | ✅ **v1.0**（转录已合并 2026-09-23，录音完整无缺口）· 讲义 50 页全覆盖 + 四个讲义外案例（Amazon / COMPAS / Apple Card / Air Canada）新增 8 个 🎙️ 小节；p.25–43 教授明说自读，已降权 |
| 5 | Accountability and Liability | | 待课件（🎙️ M04 `02:11:34` 教授预告下周主题就是 accountability） |
