...
**p.87 讨论题直接作答**（⚪ 笔记参考；讲义只给决策框架）：在同一批已核实酒店政策、相同行程约束的留出案例上，**若错误来自缺少或过时的取消政策**，优先接入可追溯的检索 / API，而不是换模型；**若证据齐全却反复漏掉“8 岁儿童 / 总价 HK\$3,000 / 免费取消”等约束**，先补明确指令与边界示例，仍持续失败才考虑微调或改模型；**若正确率合格但延迟或每接受任务成本超限**，换更轻的模型或把日期与预算判断交给确定性代码。每种改变都要在原留出案例之外再核一次，不能只修刚看过的错题。
