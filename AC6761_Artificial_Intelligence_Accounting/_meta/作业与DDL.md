...
| **拆分** | **书面报告 10% + 小组展示 10%** | ⭐ **只有讲义 p.2 有这个拆分** |
| 组队 | 3–5 人 | 官方目录 / 讲义 p.2 |
| 题目 | *"Each group is required to **examine the application of AI in accounting area**."* | 官方目录 / Outline |
| **名单 DDL** | **第 3 周末**（约 2026-09-16） | 讲义 p.2 |
| **报告 DDL** | **第 13 周末**（约 **2026-11-25**，最迟 2026-11-29） | 讲义 p.2：*"The written report (10%) should be submitted at the end of week 13"* |
| **展示时间** | **第 12–13 周**（2026-11-18 与 2026-11-25） | Outline 进度表 |
...
| **评分标准** | *"Ability to **introduce, interpret, and demonstrate** the application of AI in accounting and related functions."*<br>五档：Excellent(A) High ｜ Good(B) Significant ｜ Fair(C) Moderate ｜ Marginal(D) Basic ｜ Failure(F) Not even reaching marginal levels | 官方目录 Rubric |
...
| **时间** | **19:30 – 21:30**（2 小时） | 讲义 p.2 |
| **地点** | **AC1 (Yeung) — Y4302 & Y4702** | 讲义 p.2 |
| **形式** | **闭卷** | 官方目录 / 讲义 p.2 |
| 🔴 **范围** | **Week 1 – Week 5**（🎙️ 教授说了两遍，转录 `13:28`、`14:02`）<br>❌ **不考 Week 6 ACCESS** ❌ **不考 AI** | 转录 |
| **GenAI** | ❌ **禁止** | 官方目录 |
...
| 2 | Accounting for Business Transactions（复式记账、借贷、日记账、过账、试算表） | ⏳ 待产 |
...
gantt
    ...
    axisFormat %m-%d
    title AC6761 关键节点（日期为推算值）
    section 小组项目
    组队与选题               :active, a1, 2026-09-02, 2026-09-16
    ❗名单提交 DDL           :milestone, crit, m1, 2026-09-16, 0d
    项目研究与撰写           :a2, 2026-09-17, 2026-11-17
    小组展示（W12–W13）      :a3, 2026-11-18, 2026-11-25
    ❗书面报告 DDL           :milestone, crit, m2, 2026-11-25, 0d
    section 考试
    期中复习（W1–W5）        :b1, 2026-10-01, 2026-10-14
    ❗期中考 19:30–21:30     :milestone, crit, m3, 2026-10-14, 0d
    期末复习                 :b2, 2026-11-26, 2026-12-10
```

> 📌 **日期推算依据**：W1 = 2026-09-02（周三），逐周 +7 天，与讲义 p.2 写明的期中考 10 月 14 日（第 7 周周三）完全吻合。遇公众假期顺延，以学校课表为准。
...
| 4 | 🎙️ **教授承诺的 Word 答题模板** | 转录 `01:53:19`：*"So this word file… **we upload it**… maybe **next week** after we finish all of them."* —— 交易分析的简化答题格式模板，**W2（2026-09-09）之后去 Canvas 找**。<!-- W2转录 --> **W2 课上疑似已使用**：`20:50` *"[you can download] the format… or I will upload this [to Canvas] later"*、`35:07`–`35:21` *"follow this format… since we only have the eight [transactions], we can [delete] the others here using this format"*（模板行数多于 8）。**仍未见文件，去 Canvas 找** |
| 5 | **期末考的日期与地点** | 学校统一安排，学期后段公布 |
| 6 | **及格线的具体百分比** | 三份文件都只说"两部分都要过"，没有数字 |
| 7 | 展示的评分是个人分还是小组分 | 讲义只说"每位组员都要参与"，未说明是否分开评分 |
| 8 | **9/21 补发的四个 docx 是否计分** | `Case 1 / Case 2 with solution.docx`（带解答的练习）、`Transaction analyses for Lecture 1.docx`（W1 官方解答表）、`2 Analyze balance sheet.docx`（AI 分析提示词）——都是**带答案的练习 / 演示材料**，⚪ 推测为期中考题型预演与 #4 承诺的"答题模板"的一部分，不像计分作业；已并入 M01 / M02 / M03。若 Canvas 有对应提交入口，回填此处 |
...
| **课堂练习 2**（"chapter [2] practice"，算各账户期末余额） | ✅ `01:33:20`–`01:38:33` | *"No need to submit"*。题面只剩碎片，**解答未录到** |
| 🔴 审题 / 评分规则 | ✅ | 默认现金（`03:04`）、不写 0 扣分（`10:56`、`38:05`）、编号 / 正负号 / 备注不写 / 每笔验平衡（`34:54`–`40:49`）——已登记 [[AC6761_Artificial_Intelligence_Accounting/_meta/考点库\|考点库]] M01 段 W2-1～W2-3 |
| 上传承诺 | ✅ | `20:50` *"I will upload this [to Canvas] later"*——练习 1 的题面与格式文件。**去 Canvas 找**（渠道词听不清，按惯例应为 Canvas） |

**结论**：W2 转录**没有新增任何截止日期**；期中考仍按 W1 的 🔴（W1–W5、10-14 19:30–21:30）掌握。**要去 Canvas 找三样东西**：练习 1 题面、答题格式（疑即 Word 模板）、练习 2 题面。
