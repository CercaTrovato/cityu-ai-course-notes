...
> 课程背景见 [[Business_Data_Analytics/_prep/课程前置资料|课程前置资料]]（已读 Syllabus，**官方目录待对校**）。
...
| `Airbnb.csv` | **68,133** × 15，目标变量 `log_price` → [[Business_Data_Analytics/_meta/数据集卡片\|数据集卡片]]（W2–W4 三周共用） |
...
| `iris.txt` | 150 × 5，无表头，CRLF → [[Business_Data_Analytics/_meta/数据集卡片#iris.txt\|数据集卡片 › iris.txt]] |
...
```bash
cd "/d/上课资料/CityU/Business_Data_Analytics"
