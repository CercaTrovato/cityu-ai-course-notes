...
完整三方对校见 [[Business_Data_Analytics/_prep/课程前置资料#2. 官方目录 × Syllabus × W1 讲义 —— 三方对校总表|课程前置资料 › 2. 官方目录 × Syllabus × W1 讲义 —— 三方对校总表]]。
...
| W05 | 09-30 | ⚪ Bisecting KMeans, DBSCAN（Syllabus 原 W04，顺延待核） | Clustering Methods | — | ⏳ ⚠️ **Proposal 交（是否顺延待核）** |
...
>
> ⚠️ **2026-09-16 发现 Syllabus 与 Canvas 周次错位**：Syllabus 的 W03 = Feature Selection / PCA、W04 = Clustering，但 Canvas 的 Week 3 是描述性分析、Week 4 是特征工程——**实际进度比 Syllabus 晚一周**。W05 起按顺延填写，Proposal / Briefing 是否顺延待课上确认（[[Business_Data_Analytics/_meta/作业与DDL\|作业与DDL]] §6）。
...
| [[M03-数据类型与描述性分析]] | 对象与属性 · 四种属性类型 · 数据集类型 · 数据质量 · 描述统计（集中趋势 / 分位数 / 偏度峰度 / 协方差相关）· 相关 ≠ 因果 · **关联规则（支持度 / 置信度 / 提升度）** · 聚合与抽样 · 降维预告 · 用 Iris 画六种图 | `IS6400-W3-DataMining.pdf`（72 页；三份材料拼接） | `pending` | 名义 / 序数 / 区间 / 比率、IQR、偏度、峰度、相关系数、支持度、置信度、提升度、代表性样本、冗余特征 |
...
| [[T02-回归实战-从合成数据到Airbnb定价]] | sklearn `LinearRegression` · R²/MAE · 多项式过拟合 · `Ridge` · `OneHotEncoder` · `train_test_split` · **Airbnb 房价预测** · **1 个整段 AI Prompt** | `Week 2 Regression Analysis_With Prompt.ipynb`（39 cells） | [[Business_Data_Analytics/_meta/数据集卡片#Airbnb.csv\|数据集卡片 › Airbnb.csv]] |
| [[T03-数据探索实战-Iris与Airbnb的描述统计]] | 描述统计函数 · 分位数 / 偏度峰度 · 分组 · 四种图 · **缺失值三种填补** · **IQR vs Z 离群点** · StandardScaler / RobustScaler · 相关矩阵热力图 · **Week 3 Assignment（3 题 100 分）** · ⚠️ 无 AI Prompt 格 | `Week 3 Description.ipynb`（48 cells） | [[Business_Data_Analytics/_meta/数据集卡片#iris.txt\|iris.txt]] · Airbnb.csv |
...
| 文件 | 作用 |
|---|---|
| [[Business_Data_Analytics/_meta/知识层级台账\|知识层级台账]] | ⭐ **写笔记前必读**。L0 入场基线（**本课的 Python 口径写在这里**）+ L1 概念登记表 |
| [[Business_Data_Analytics/_meta/术语表\|术语表]] | 本课双语术语，含考试可用的英文定义 |
| [[Business_Data_Analytics/_meta/考点库\|考点库]] | 按 module 汇总的潜在考点，带三档可信度 |
| [[Business_Data_Analytics/_meta/作业与DDL\|作业与DDL]] | 所有作业、截止日期、提交格式 |
| [[Business_Data_Analytics/_meta/数据集卡片\|数据集卡片]] | `Airbnb.csv` 与 `iris.txt` 的六项数据集卡（真实跑出来的统计） |
| [[Business_Data_Analytics/_meta/待并入术语总表\|待并入术语总表]] | 需由主 agent 合并进根 `_meta/术语总表.md` 的跨课条目 |
| [[Business_Data_Analytics/_prep/课程前置资料\|课程前置资料]] | 官方目录 × Syllabus × 讲义三方对校；所有硬规则的出处 |
...
| 算法偏见 / 数据代表性 | [[Business_Data_Analytics/_meta/数据集卡片#5. 已知坑\|数据集卡片 › 5. 已知坑]]（Airbnb 只有 6 个城市、property_type 只剩 4 类） | IS5113 W03 偏见与公平 |
...
将来的转录按 [[转录处理规则#5. 文件约定|转录处理规则 › 5. 文件约定]] 命名为 `transcripts/M<NN>-transcript.txt`。
...
| 8 | **W03 转录** | M03 / T03 的 22 + 0 处 🎙️ 格全空；考点 🟡 封顶；关联规则是否考试范围未知 | 9/16 已上课，等用户导出 Notta |
...
| 11 | **Syllabus 周次顺延** | W05 起主题、Proposal / Briefing 日期 | 课上确认 |
...
- 课程背景与三方对校：[[Business_Data_Analytics/_prep/课程前置资料|课程前置资料]]
- 连贯性机制：[[Business_Data_Analytics/_meta/知识层级台账|知识层级台账]]
