...
> **原始材料**：`IS6400-W3-DataMining.pdf`（72 页，Canvas 标题 "Week 3 - Descriptive Analytics"）｜ **配套 tutorial**：[[T03-数据探索实战-Iris与Airbnb的描述统计]] ｜ **指定阅读**：DM Chapter 3 ｜ **转录**：`pending`（W03 于 9/16 上课，录音尚未导出；🎙️ 各格暂写"待转录补充"，考点 🟡 封顶）
>
...
| 均值、中位数、方差、标准差、分位数、正态分布、相关性的直觉 | L0-A 统计基线。**本讲会给正式定义和讲义原句**，但不从零讲什么是平均数 | [[Business_Data_Analytics/_meta/知识层级台账\|知识层级台账]] L0-A |
...
| Airbnb 数据集 | 68,133 行 × 15 列，目标 `log_price`；本讲 p.5 用它开场 | [[Business_Data_Analytics/_meta/数据集卡片#Airbnb.csv\|数据集卡片 › Airbnb.csv]] |
...
| **判断"哪些列不是属性"** | `Tid` 是给行编号的，它是**标识符**，不是描述对象的性质；Airbnb 的 `id` 同理。这类列必须在建模前排除（[[Business_Data_Analytics/_meta/数据集卡片\|数据集卡片]] §5.7 已经踩过一次：`id` 是递增的，会与"上架时间"虚假相关） |
...
把它套到 Airbnb（[[Business_Data_Analytics/_meta/数据集卡片#Airbnb.csv\|数据集卡片 › Airbnb.csv]]）——**这就是 T03 作业第 1 题的答案骨架，但每一条理由要你自己写**：
...
[[Business_Data_Analytics/_meta/数据集卡片\|数据集卡片]] 里的实跑数字：**质量**——`review_scores_rating` 缺 15,275 个（22.4%），`host_has_profile_pic` / `host_identity_verified` 各缺 180 个，且是同一批行；离群点按 1.5×IQR 规则 `log_price` 标出 1,453 个；重复行 0。**维度**——15 列；把 `property_type`（4 类）、`city`（6 类）、`cancellation_policy`（5 类）、`bed_type`（5 类）one-hot 之后变成 15 − 4 + 20 = 31 列。**稀疏**——one-hot 出来的 20 列里每行只有 4 个 1、16 个 0。**分辨率**——按房源看 `log_price` 标准差 0.71，按城市取中位数后六个数一目了然（SF 5.075 最高，T03 §3.1）。
...
**数字例子**（Airbnb `city`，[[Business_Data_Analytics/_meta/数据集卡片\|数据集卡片]] §2.2 实跑）：
...
| 探测多重共线性 | M02 里 `beds` 系数为负的原因：`accommodates` 与 `beds` 相关 0.829（[[Business_Data_Analytics/_meta/数据集卡片\|数据集卡片]] §5.1）——建模前看一眼相关矩阵就能预见 |
...
**Iris 的结构**（[[Business_Data_Analytics/_meta/数据集卡片#iris.txt\|数据集卡片 › iris.txt]] 实跑）：150 行 × 5 列；4 个比率属性 + 1 个名义类别；三类各 50 朵；无缺失。按品种的均值——
...
| 级别 | 含义 | 本讲 |
|---|---|---|
| 🔴 教授明示 | 转录里教授明确说过会考 / 要记 | **暂无**（W03 录音未导出，🟡 封顶） |
...
**总页数：72 页（页脚自编号到 73/73，原稿少一页；p.35–49 页脚为 "36/42…50/42"，来自另一份讲义）。✅ 71 页在 §2 有实质讲解；例外 p.1（封面）。** 本讲无转录，「课堂覆盖」整列写 `—`。

| 笔记小节 | 讲义页 | 内容 | 课堂覆盖 |
|---|---|---|---|
| —（封面） | p.1 | 标题页：Lecture 3 · Data Types 红字 · Reading: DM Chapter 3 | — |
| §2.1 | p.2 | CRISP-DM 圆环，红框圈 2 Data Understanding + 3 Data Preparation | — |
| §2.1 | p.3 | HM Figure 1-2 机器学习流程，红框圈 Data | — |
| §2.2 | p.4 | 对象与属性的定义 + Tid/Refund/Marital/Income/Cheat 表 | — |
| §2.1 | p.5 | Airbnb `read_csv` + `head()` 截图 | — |
| §2.1 | p.6 | 超市 ERP 表（客户 / 门店 / 交易三段） | — |
| §2.2 | p.7 | 属性值；同一属性不同值、不同属性同一值 | — |
| §2.3 | p.8 | 名义、序数 | — |
| §2.3 | p.9 | 区间、比率 | — |
| §2.3 | p.10 | 四种性质与四种类型的对应 | — |
| §2.4 | p.11 | 离散 / 连续 | — |
| §2.4 | p.12 | 非对称名义（二元）属性 | — |
| §2.3 | p.13 | Tid / Refund / Marital 名义，Income 比率 | — |
| §2.5.2 | p.14 | 数据集三大类九小类 | — |
| §2.5.1 | p.15 | 数据矩阵 | — |
| §2.5.1 | p.16 | 文档数据（词频向量） | — |
| §2.5.1 | p.17 | 交易数据 | — |
| §2.5.2 | p.18 | 图数据（合著者网络、Facebook） | — |
| §2.6.1 | p.19 | 数据质量三问三例 | — |
| §2.6.2 | p.20 | 结构化数据四特征 | — |
| —（章节标题页） | p.21 | 标题页重贴，红字 Descriptive Analytics · Simple Statistics（已在 §2.6 末尾点明） | — |
| §2.7 | p.22 | 描述性分析定义与三种方式 | — |
| §2.8 | p.23 | 类别变量：计数 | — |
| §2.9.1 | p.24 | 数值变量统计量清单 | — |
| §2.9.2 | p.25 | 百分位数与四分位数 | — |
| §2.9.3 | p.26 | 极差与 IQR（9 个数的例子） | — |
| §2.10 | p.27 | 偏度（**纯图片**，三条曲线，已复核） | — |
| §2.10 | p.28 | 峰度（**纯图片**，三条曲线，已复核） | — |
| §2.11 | p.29 | 关联度量：49 所大学的数据 | — |
| §2.11 | p.30 | 协方差（总体 / 样本公式） | — |
| §2.11 | p.31 | 协方差 Excel 算例（**近纯图片**，已复核：cov = 263.37） | — |
| §2.11 | p.32 | 相关系数公式 | — |
| §2.11 | p.33 | 四种散点图（含非线性零相关） | — |
| §2.12 | p.34 | 相关 ≠ 因果；实验 / 准实验 | — |
| §2.13 | p.35 | 关联规则开场：购物车图 | — |
| §2.13 | p.36 | 定义 + 5 笔交易表 + 三条规则例 | — |
| §2.13 | p.37 | 四个层次的关联 | — |
| §2.13 | p.38 | 为什么在乎：货架 / 推荐 / 捆绑 | — |
| §2.13 | p.39 | 项集、支持度计数、支持度、频繁项集 | — |
| §2.14.1 | p.40 | 规则定义、s 与 c、{Milk, Diaper} → {Beer} 算例 | — |
| §2.14.1 | p.41 | 支持度（4 笔交易表） | — |
| §2.14.1 | p.42 | 置信度 | — |
| §2.14.2 | p.43 | 有效规则：两个门槛 + 电话公司例 | — |
| §2.14.2 | p.44 | minsup/minconf = 50% 走一遍 | — |
| §2.14.2 | p.45 | 支持度与置信度的含义 | — |
| §2.15 | p.46 | 茶 → 咖啡：置信度 75% | — |
| §2.15 | p.47 | 茶 → 咖啡：与基准 90% 比较 | — |
| §2.15 | p.48 | 提升度 | — |
| §2.15 | p.49 | Exercise（实为 p.40 算例重贴） | — |
| —（章节标题页） | p.50 | 标题页重贴，红字 Data Processing（已在 §2.15 末尾点明） | — |
| §2.16.1 | p.51 | 聚合 | — |
| §2.16.1 | p.52 | 澳大利亚降水（**纯图片**两张直方图，已复核） | — |
| §2.16.2 | p.53 | 抽样 | — |
| §2.16.2 | p.54 | 代表性 | — |
| §2.16.2 | p.55 | 样本大小（**纯图片**，8000/2000/500 点，已复核） | — |
| §2.16.2 | p.56 | 抽样类型 | — |
| §2.17 | p.57 | 降维 | — |
| §2.17 | p.58 | 特征子集选择 | — |
| §2.17 | p.59 | 特征创建 | — |
| §2.18.1 | p.60 | 投影危险五帧（**纯图片**，已复核） | — |
| §2.18.1 | p.61 | Iris 数据集 | — |
| §2.18.1 | p.62 | Iris 照片（**纯图片**，标 sepal / petal，已复核） | — |
| §2.18.2 | p.63 | 直方图（10 / 20 箱） | — |
| §2.18.2 | p.64 | 二维直方图 | — |
| §2.18.3 | p.65 | 箱线图结构 | — |
| §2.18.3 | p.66 | Iris 箱线图（**纯图片**，已复核） | — |
| §2.18.4 | p.67 | 散点图 | — |
| §2.18.4 | p.68 | Iris 散点图矩阵（**纯图片**，已复核） | — |
| §2.18.5 | p.69 | 平行坐标 | — |
| §2.18.5 | p.70 | Iris 平行坐标两种轴序（**纯图片**，已复核） | — |
| §2.18.5 | p.71 | 星图与 Chernoff 脸（**纯图片**，已复核） | — |
| §2.18.5 | p.72 | Iris 星图（**纯图片**，已复核） | — |
...
本讲无转录，无法比对。⚠️ 以下几处**很可能**有课堂口述：p.6 超市案例"老板要什么报告"、p.19 数据质量三问的答案、p.49 "Exercise" 页（讲义上没有题目，教授可能现场出题）、p.64 "What does this tell us?"。
...
| **本课的 Airbnb 数字** | T03 notebook 的所有输出（分位数、缺失、离群点计数、相关矩阵）已用 `verify_airbnb.py` 在 68,133 行原始数据上复核一致；数据集卡片见 [[Business_Data_Analytics/_meta/数据集卡片\|数据集卡片]] | 本库 |
...
| ② | **转录** | 本讲录音尚未导出。到位后回填 22 处"待转录补充"、§8 课堂覆盖列、§9.1–9.2，并把教授强调的条目升为 🔴 |
| ③ | **考试范围是否含关联规则** | 讲义标题页写 DM Chapter 3，但 p.35–49 的关联规则在 Tan 等教材里是第 6 章；Syllabus 的周次主题里也没有 Association Rule。⚪ 建议课上直接问 |
| ④ | **Syllabus 周次顺延** | 实际 W3 = 描述性分析、W4 = 特征工程，比 Syllabus 各晚一周；W05（分类）、W06（Briefing）是否顺延**直接影响 Proposal 截止日**——见 [[Business_Data_Analytics/_meta/作业与DDL\|作业与DDL]] §2 |
