...
| 表、字段、行、SQL 的 SELECT 语义 | L0 入场基线里已有的直觉 | [[Artificial_Intelligence_Accounting/_meta/知识层级台账\|知识层级台账]] L0 |
...
| 2 | 🔴 **译名冲突已确认：引用完整性（M04 §2.9.5）vs 参照完整性（本讲 §2.4.3）** | M04 确实有 §2.9.5「引用完整性实例（讲义 p.40）」一节。**同一个 referential integrity，本课两讲用了两个中文名。** 本笔记选「参照完整性」的理由：**它对齐 Access 中文版界面上的按钮文字**，学生上机时看到的就是这四个字。⚠️ **须由主 agent 裁决并全库统一**，已登记在 [[Artificial_Intelligence_Accounting/_meta/待并入术语总表\|待并入术语总表]] 的 W6-D 区块 |
...
- 台账：[[Artificial_Intelligence_Accounting/_meta/知识层级台账|知识层级台账]] ｜ 术语：[[Artificial_Intelligence_Accounting/_meta/术语表|术语表]] ｜ 考点：[[Artificial_Intelligence_Accounting/_meta/考点库|考点库]]
