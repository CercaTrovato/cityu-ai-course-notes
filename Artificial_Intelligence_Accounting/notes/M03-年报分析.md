...
> 2. **W3 讲义从 `49:04` 起讲**（p.2 年报组成），但**录音在 `01:54:24` 结束时只讲到 p.27**（Nike 共同比案例的两道练习题答案）。**p.28 起（利润表、现金流量表、权益变动表、附注、§2.10 共 20 页）本次录音完全没有覆盖**，正文 §2.5–§2.10 的 🎙️ 格因此全部是 ❓ 而不是 ⏭️。
...
> 🎙️ **课堂实况**（2026-09-16）：教授先用约 49 分钟补完 W2 顺延内容——课堂练习 2 的解答（`00:01`–`17:43`）、当堂小测（`17:43`–`40:14`，收上来作课堂参与证据）、试算表概念与 AI 演示（`40:14`–`49:04`）；**W3 讲义本身只用了约 65 分钟**（`49:04`–`01:54:24`），从年报组成一路讲到 p.27 的 Nike 共同比案例，中间约 6.6 分钟（`01:40:23`–`01:47:02`）是学生自读 p.20–25 的时间。**重心在资产负债表逐项讲解（§2.2，约 34 分钟）与一段 11.5 分钟的 AI 分析资产负债表课堂演示**；p.28 起的利润表、现金流量表、权益变动表、附注**全部没有录到**。所以本讲课堂的重点是"资产负债表怎么读 + AI 能不能自动分析"，利润表之后的内容只能靠讲义与本笔记自学。
...
**🎙️ 课堂补充**：❓ **录音范围内没有讲到 p.29（利润表定义、四要素公式）。** 转录在 `01:54:24` 结束，最后覆盖到的讲义内容是 p.27 的 Nike 练习题答案（`01:47:42`–`01:54:24`），此后再无录音。**不能标成 ⏭️ 略过**——按 [[转录处理规则]] §5，没录到 ≠ 没讲；从进度看（`49:04` 才开始讲 W3 讲义、`01:54:24` 只到 p.27），教授大概率是下一次课才讲到这里。本节内容只能靠讲义与本笔记自学，不要因为"课上没听到"而降低优先级。
...
**🎙️ 课堂补充**：❓ **录音范围内没有讲到 p.30（利润表标准结构、EPS）。** 同 §2.5.1，转录 `01:54:24` 结束时仍停在 p.27，本页在录音范围之外。**不可标 ⏭️**——没录到 ≠ 没讲。本节内容只能靠讲义与本笔记自学。
...
**🎙️ 课堂补充**：❓ **录音范围内没有讲到 p.31–32（收入侧与费用侧明细）。** 同上，本讲录音在讲到 p.27 后即结束，p.28 起全部没有覆盖。**不可标 ⏭️**——没录到 ≠ 没讲。本节内容只能靠讲义与本笔记自学。
...
**🎙️ 课堂补充**：❓ **录音范围内没有讲到 p.33（利润表 Key information 与 Weaknesses）。** 本页是本讲三大论述题高发页之一，但录音只到 p.27。**不可标 ⏭️**——没录到 ≠ 没讲，且这一页的重要性不因没录到而降低，反而要更重视自学。
...
**🎙️ 课堂补充**：❓ **录音范围内没有讲到 p.34（A 公司真实利润表）。** 这一页是纯图片、没有讲义文字，本讲最需要转录佐证的一处，但录音在讲到 p.27 后就结束了，完全没有覆盖。**不可标 ⏭️**——没录到 ≠ 没讲，本节的"利润腰斩之谜"分析（§2.5.5）目前仍是 💡 笔记推断，未获课堂验证也未被推翻。
...
**🎙️ 课堂补充**：❓ **录音范围内没有讲到 p.37（Nike 共同比利润表）。** 本讲录音只到 p.27（§2.4.6），没有进入 §2.6 Nike 利润表案例。**不可标 ⏭️**——没录到 ≠ 没讲。本节"净利率上升主要来自税率下降"这条判断只能靠讲义与本笔记自学，尚未获课堂验证。
...
**🎙️ 课堂补充**：❓ **录音范围内没有讲到 p.38（Nike 收入确认与需求创造费用政策）。** 同 §2.6.2，本讲录音没有覆盖到这一页。**不可标 ⏭️**——没录到 ≠ 没讲。本节内容只能靠讲义与本笔记自学。
...
**🎙️ 课堂补充**：❓ **录音范围内没有讲到 p.40（直接法与间接法）。** 本讲录音结束于 `01:54:24`，只讲到 p.27，现金流量表一节（§2.7）完全没有被录到。**不可标 ⏭️**——没录到 ≠ 没讲。本节内容只能靠讲义与本笔记自学。
...
**🎙️ 课堂补充**：❓ **录音范围内没有讲到 p.41（CFO / CFI / CFF 三类定义）。** 同 §2.7.1，本讲录音没有覆盖到现金流量表一节。**不可标 ⏭️**——没录到 ≠ 没讲。本节内容只能靠讲义与本笔记自学。
...
**🎙️ 课堂补充**：❓ **录音范围内没有讲到 p.42（现金流量表两大用途）。** 本页是本讲三大论述题高发页之一，但同样没有被录到——本讲录音在讲到 p.27 后即结束。**不可标 ⏭️**——没录到 ≠ 没讲，重要性不因没录到而降低。
...
**🎙️ 课堂补充**：❓ **录音范围内没有讲到 p.43–44（A 公司真实现金流量表）。** 这两页是纯图片、没有讲义文字，本应是本讲最需要转录佐证的地方之一，但录音在讲到 p.27 后就结束了。**不可标 ⏭️**——没录到 ≠ 没讲，§2.10 那个"现金对不上"的经典勾稽练习目前仍是 💡 笔记推断，未获课堂验证。
...
**🎙️ 课堂补充**：❓ **录音范围内没有讲到 p.45（权益变动表：缴入资本 vs 赚得资本）。** 本讲录音没有覆盖到权益变动表一节（§2.8）。**不可标 ⏭️**——没录到 ≠ 没讲。本节内容只能靠讲义与本笔记自学。
...
**🎙️ 课堂补充**：❓ **录音范围内没有讲到 p.46（A 公司真实权益变动表）。** 这一页是纯图片、13 列全表，是四张表里最复杂的一张，本应最需要课堂讲解，但录音在讲到 p.27 后就结束了。**不可标 ⏭️**——没录到 ≠ 没讲，§2.8.2 的四次验算目前仍是 💡 笔记推断，未获课堂验证。
...
**🎙️ 课堂补充**：❓ **录音范围内没有讲到 p.47（报表附注：《公司条例》、Note 1 会计政策）。** 本讲的最后一页，也是本讲录音完全没有覆盖到的部分。**不可标 ⏭️**——没录到 ≠ 没讲。本节内容只能靠讲义与本笔记自学。
...
**🎙️ 课堂补充**：❓ **录音范围内没有做过这类勾稽演示。** 本节本来就没有对应的讲义页（💡 笔记补充），本讲录音也只讲到 p.27，从未进入四张真实报表相互勾稽的环节。**不可标 ⏭️**——教授是否在别的课时做过这类演示，转录到手后核对；本节的三条勾稽线目前仍是 💡 笔记验算，未获课堂验证。
...
| 级别 | 含义 | 本讲数量 |
|---|---|---|
| 🔴 教授明示 | 转录里教授明确说过会考 / 要记 / 讲义故意留白但当堂给了答案 —— **引用原话** | **3 条 + 1 条 🔴（反向）**（本讲转录 `M03-transcript.txt`） |
...
> 逐页可查。「类型」列标出**封面页**与**分隔页**；「课堂覆盖」列填的是本次转录（`M03-transcript.txt`）实际讲到的程度——**本讲录音只讲到 p.27**，p.28 起全部标 ❓。
...
| p.28 | **分隔页** | Income Statement (Statement of profit or loss) | §2.5 开头 | — | ❓ **录音在 `01:54:24` 结束前没有讲到这里** |
| p.29 | 内容 | 利润表定义；`NI = Rev − Exp + Gains − Losses` | §2.5.1 | — | ❓ 未录到 |
| p.30 | 内容 | 多步式利润表的完整结构、EPS | §2.5.2 | — | ❓ 未录到 |
| p.31 | 内容 | 收入侧：Revenue、Gross profit、Other revenues | §2.5.3 | — | ❓ 未录到 |
| p.32 | 内容 | 费用侧：Distribution / Administration / Others / Finance | §2.5.3 | — | ❓ 未录到 |
| p.33 | 内容 | **利润表 Key information 与 Weaknesses** | §2.5.4 | — | ❓ 未录到 |
| **p.34** | **图片页** ✅已视觉复核 | A 公司 2019 合并损益表 | §2.5.5 | — | ❓ 未录到 |
| p.35 | **分隔页** | Case: Nike — Analyzing Income Statement | §2.6 开头 | — | ❓ 未录到 |
| **p.36** | **图片页** ✅已视觉复核 | Nike 利润表 2014–2016 | §2.6.1 | — | ❓ 未录到 |
| **p.37** | **图片页** ✅已视觉复核 | Nike 共同比 + 百分比变动利润表 | §2.6.2 | — | ❓ 未录到 |
| p.38 | 内容 | Nike 附注摘录：收入确认政策、需求创造费用 | §2.6.3 | — | ❓ 未录到 |
| p.39 | **分隔页** | Cash Flow Statement | §2.7 开头 | — | ❓ 未录到 |
| p.40 | 内容 | 现金流量表定义；直接法 vs 间接法 | §2.7.1 | — | ❓ 未录到 |
| p.41 | 内容 | CFO / CFI / CFF 三类定义 | §2.7.2 | — | ❓ 未录到 |
| p.42 | 内容 | **现金流量表的两大用途** | §2.7.3 | — | ❓ 未录到 |
| **p.43** | **图片页** ✅已视觉复核 | A 公司 2019 合并现金流量表（经营活动） | §2.7.4 | — | ❓ 未录到 |
| **p.44** | **图片页** ✅已视觉复核 | A 公司 2019 合并现金流量表（投资与筹资活动） | §2.7.4 | — | ❓ 未录到 |
| p.45 | 内容 | 权益变动表：缴入资本 vs 赚得资本 | §2.8.1 | — | ❓ 未录到 |
| **p.46** | **图片页** ✅已视觉复核 | A 公司 2019 合并权益变动表（13 列全表） | §2.8.2 | — | ❓ 未录到 |
| p.47 | 内容 | 报表附注：《公司条例》、Note 1 会计政策 | §2.9 | — | ❓ 未录到 |
...
**课堂覆盖统计**（讲义 47 页，用 `_meta/tools/transcript_check.py` 的时间戳解析人工核对）：✅ 讲了 **21 页**（p.2, p.4–18, p.26–27，其中 p.13 后附加了 AI 演示）｜ ⏭️ 自读为主（教授明说） **6 页**（p.20–25）｜ ❓ 未录到 **19 页**（p.28–47，即 §2.5–§2.9 全部）｜ ⚪ 封面/分隔页 **1 页封面 + 5 页分隔页均已在 §2 对应小节说明**。
...
| **p.28–47（§2.5–§2.9，共 19 页）** | 利润表、现金流量表、权益变动表、附注四整节 | ❓ **录音缺失** | 转录 `01:54:24` 结束，最后覆盖到的讲义页是 p.27（`01:54:10`），之后再无录音；`49:04` 才开始讲 W3 讲义，进度明显慢于往常 | **不可降权**——这四节是四张报表体系里最重要的三张（利润表/现金流量表/权益变动表），且现金流量表、利润表都是本讲三大论述题高发页所在，只能靠讲义与本笔记自学 |
| — | 四张报表的勾稽验算（§2.10，讲义本来就没有对应页） | ❓ 录音没有做这类演示 | 同上，录音从未进入 §2.5 之后的内容 | 不可降权，💡 笔记验算保留 |
...
> ✅ **v0.9 → v1.0 已完成**：32 处「🎙️ 课堂补充」占位格全部落到 A/B/C/D 四态之一，见 §9.6 变更记录。frontmatter 已改 `transcript: merged`、`status: v1.0`；§6.1 已重做分级（3 条 🔴 + 1 条 🔴（反向））。
...
| 8 | 教授是否口头补充了权益变动表 | ❓ **未获转录证据**——录音在讲到 p.27 后结束，从未进入 §2.8。讲义只有 1 页文字（§9.3 ⑨）这一疑点维持不变 |
| 9 | 小组名单（3–5 人）的**提交渠道** | ❓ **转录里没有出现渠道说明**。唯一相关的一句在末段被截：`01:54:24` *"if you didn't send a group name"*——只提到"名字"，没有说提交到哪里。[[Artificial_Intelligence_Accounting/_meta/作业与DDL\|作业与DDL]] 记录的截止日期"第 3 周末"仍待去 Canvas 确认渠道 |
