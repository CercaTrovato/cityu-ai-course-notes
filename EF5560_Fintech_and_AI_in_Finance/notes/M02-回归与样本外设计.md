...
L0 层面另需：最小二乘、R²、标准误、p 值的基本含义（见 [[EF5560_Fintech_and_AI_in_Finance/_meta/知识层级台账|台账]] L0）。**但"金融里怎么读它们"不在 L0**，本讲展开。
...
1. **五个最大绝对值收益是谁？** 以课程 `spy_monthly_features_240m.csv` 的 `market_return` 列为例，先按绝对值排序，再保留原符号：**2008-10 −18.055%、2020-03 −13.338%、2020-04 +11.954%、2009-02 −11.367%、2011-10 +10.359%**（数据列为课程月度对数收益，数值 ×100 后四舍五入到 0.001 个百分点；不同市场会有不同名单）。`code/L01_data/check_exercises.py` 可复算；这不是讲义给出的标准名单。
2. **应该因为它们“太大”而删掉吗？** **不应该。** 教授明确要求保留真实危机和反弹月份（P1 `01:02:16`–`01:04:00`）；仅凭离均值远、超过三个标准差或影响回归结果都不是删除理由，否则模型恰好学不到极端市场状态。
3. **什么证据才支持剔除？** 逐条回原始价格、复权 / 拆股 / 派息记录、交易日历和数据源核对。若发现错误代码、未复权造成假跌幅、重复行、日期错配或供应商录入错误，才可修正或剔除，并记录原值、原因和处理前后结果。**真实发生的极端收益应保留**，必要时另报含 / 不含极端月的敏感性分析，不能静默删数。
...
**💡 「kitchen sink」这个词（笔记补充）**：英文习语 "everything but the kitchen sink" = "除了厨房水槽什么都带上了"，形容不加选择地全塞进去。**Welch & Goyal 用它来讽刺"变量越多越好"的做法**，它现在是资产定价文献里的标准术语。→ 记进 [[EF5560_Fintech_and_AI_in_Finance/_meta/术语表|术语表]]。
...
> ★ **跨课注意**：IS6400 W02 的通用做法可能在全样本上标准化（`StandardScaler().fit_transform(X)`）。**在本课语境下那是泄漏。** 见 [[EF5560_Fintech_and_AI_in_Finance/00-课程总览|00-课程总览]] 的跨课对照表。
...
> 💡 这条脚注很诚实——**它自己承认三列不完全可比**。这与 [[EF5560_Fintech_and_AI_in_Finance/_meta/数据集卡片|数据集卡片]] §4 实测到的"SPY 20 年 8.44 倍 vs S&P 500 价格指数 5.87 倍"是同一件事。
...
**按五项证据作答（⚪ 笔记参考演练；选择“12 个月动量预测下月 SPY 收益”，不是讲义附的官方答案）**：

1. **两张图**：讲义 p.12 的 SPY 散点在噪声里看不出清晰斜率，等频分箱后仅有轻微向上的形状（§2.3.2–2.3.3）；分箱使均值变平滑，不能掩盖原始点的巨大散布。
2. **两张系数表**：简单回归动量斜率 **0.6 pp**、$p=0.76$、样本内 $R^2=0.1\%$（p.24）；把其他变量一同放入后，标准化动量斜率 **0.4%**、$p=0.50$（p.33）。两表的斜率单位不同，不能把 0.6 和 0.4 直接当成系数变化幅度；两处不确定性都大，尚无强证据表明动量单独有效。
3. **样本外记分卡与基准**：讲义 p.46 的 **+6.6% OOS $R^2$** 是**三个预测变量共同组成的 SPY 回归**相对滞后 MA(12) 的成绩，不是动量单变量的成绩；不能把联合模型的改进归功于动量。若要判断“动量一个变量能否预测”，需在同一时间切分下另做预先确定的动量单变量样本外预测，再对 MA(12) 评分。
4. **为何预测精度不等于投资价值**：即使 OOS $R^2>0$，它只表示数值误差比 MA(12) 小；方向命中、仓位约束、交易成本与风险可能使策略仍不值得做。这里联合模型在 60 个测试月里每月都预测正收益，方向判断只与“永远猜涨”打平（§2.7.4），更不能据此宣称动量策略赚钱。
5. **平实结论与改变结论的证据**：目前只看到“动量图形略有上升，但单变量回归很弱”；对“动量单独有预测价值”保持保留。若**事先固定**动量定义和切分，随后独立时间段的单变量 OOS $R^2$ 稳定为正，且方向 / 扣费后组合收益胜过各自合适的基准，我才会提高信心；若该检验为负，即使样本内图更漂亮也不改判。不能用同一冻结测试集反复挑结果。
💡 这五样就是 ML 投资作业报告的骨架（[[EF5560_Fintech_and_AI_in_Finance/_meta/作业与DDL|作业与DDL]] §2.2）。
...
**⑥ 缺少的东西**
- **可复现代码包**（作业硬性要求，见 [[EF5560_Fintech_and_AI_in_Finance/_meta/作业与DDL|作业与DDL]] §2.2）
- **组合权重 CSV**（同上）
...
见 [[EF5560_Fintech_and_AI_in_Finance/00-课程总览|00-课程总览]] 的跨课对照表：R² 的好坏标准相反｜标准化的时机不同｜benchmark ≠ baseline。**IS6400 的通用回归讲法可以直接借来理解数学，但它的三条工程习惯在本课语境下都是错的。**
...
| 4 | `cli_gap_lag2` 的确切定义 | 讲义 p.34 说是「OECD composite leading-indicator gap **from 100**」，比数据文件里的裸列名清楚，但**原始序列与单位仍需向 TA 确认**。见 [[EF5560_Fintech_and_AI_in_Finance/_meta/数据集卡片\|数据集卡片]] §8 |
...
- [[EF5560_Fintech_and_AI_in_Finance/_meta/数据集卡片|数据集卡片]] §7（`spy_monthly_features_240m.csv`）、§8（`csi300_macro_panel.csv`）——本讲的两个主力数据集
- [[EF5560_Fintech_and_AI_in_Finance/_meta/作业与DDL|作业与DDL]] §2.2（ML 投资作业用的就是本讲的评价标准）
- **跨课 ★★**：[[M02-预测分析-线性回归]]（IS6400 W02）—— **同周、同主题、同样在教用 AI 写代码**。分工很清楚：**IS6400 讲数学**（正规方程、极大似然、梯度下降、Lasso/Ridge、虚拟变量陷阱），**本讲讲纪律**（按时间切分、只用训练集标准化、冻结规格、样本外 R² 与基准命中率）。**IS6400 的正则化那一节正好是本课 Class 3 的预习**（讲义 L2 p.28 / p.30 两次预告）。三处口径冲突（R² 好坏标准 / 标准化时机 / benchmark ≠ baseline）见 [[EF5560_Fintech_and_AI_in_Finance/00-课程总览|00-课程总览]]
