...
| 4 | ❗ **`shared/` 四个文件仍然缺失** | `class03/README.md` 引用 `../shared/market_excess_return_panel.csv`、`hsi_stock_excess_return_panel.csv`、`technical_characteristics_dictionary.csv`、`risk_free_yields_monthly.csv`，导出包与转录里都没有出现这四个文件；`01:18:56` 教授只说数据在 Dropbox / Canvas，没有点名 `shared/`。仍需向教授 / TA 索要；p.35 / p.40 / p.43 精确数字暂无法本地复现 |
...
| 7 | ✅ 所有真实数字已复核 | p.14、21、23、47、49、55 的每个数与 `class03/` 五个 CSV 逐格一致（`code/L03_linear_ml/01_reproduce_lecture.py` B 段，`code/run_all.py` 11/11 通过） |
