...
> **原始材料**：`Lec03_Linear_Machine_Learning.pdf`（56 页）+ `course_files_export/class03/class03/`（5 个 CSV + `manifest.json` + `README.md`）｜**转录**：`merged`（`M03-transcript.txt`，`00:56 → 02:24:04`，587 段，2026-09-17 合并）｜**状态**：**v1.0**（按 [[笔记质量规范]] v1 写，strict 模式）

> ✅ **本笔记为 v1.0**（2026-09-17 讲义版 v0.9 → 2026-09-17 合并转录）。**先知道这份转录的 4 个特殊之处，再读正文**：
> 1. **缺开头约 1 分钟**——首段 `00:56` 已是教授在回答学生关于 syllabus 的提问；`00:56`–`08:32` 是行政段（TA tutorial、期末考形式、评分构成、随机分组），讲义无对应页，已提取到 [[EF5560_Fintech_and_AI_in_Finance/_meta/作业与DDL|作业与DDL]]、[[EF5560_Fintech_and_AI_in_Finance/_meta/考点库|考点库]] 与 §9.2。
> 2. **缺结尾**——`02:23:49` *"let me summarize with the last slide"* 总结刚开始，`02:24:04` 一句后中断，p.56 的当堂口头总结与 Class 4 预告丢失；M04 开头仅部分回顾线性模型和非线性动机，不能恢复 M03 结尾原话（§2.9.4、§9.1 仍标 ❓）。
> 3. **无 W2 顺延内容**——`08:32` *"come back to the slide"* 起直接进入本讲正文，不需要回填 M02。
> 4. **讲课顺序与讲义页码顺序不完全一致**——教授在 Ridge / LASSO / Elastic Net 三节之间穿插了同一段现场 AI（vibe coding）演示（`47:31`–`01:07:07`，对应讲义 p.25 的代码页）；本笔记仍按讲义小节回填，🎙️ 格里注明了真实时间戳。
>
...
> 🎙️ **课堂实况**（2026-09-17 周四课）：教授先花约 7.5 分钟处理行政事务（`00:56`–`08:32`：TA tutorial 安排、期末考形式、评分构成、随机分组），随后进入本讲正文，从市场任务的收缩方法一路讲到个股任务的降维与排序，中间穿插约 20 分钟的职业闲聊与人生感悟（`58:22`–`01:03:36`、`01:12:45`–`01:14:37` 等，讲义无对应页）；**重心明显在 Ridge / LASSO / Elastic Net 三个模型的现场 AI 演示**（`44:47`–`01:07:07`，约 22 分钟，边讲公式边用连着 Dropbox 的 AI 工具跑出结果表）与个股任务的排序证据（`01:48:10`–`01:55:31`：动量分组、波动率"反直觉"发现、前视偏差告诫）。**M03 当堂 p.56 的总结与 Class 4 预告没有录到**（`02:23:49` 起被截断）；M04 A `04:00`–`13:23` 部分回顾高维、简单基准与非线性动机，但未复述 p.56 的全部个股排序结论。所以本讲课堂的重点仍是"怎么用 AI 现场跑收缩模型、以及排序为什么在个股任务上比预测精度更有用"；p.56 原始课堂缺口没有被完全恢复。
...
**🎙️ 课堂补充**（`08:47`–`10:12`，约 1.5 分钟，A · 课上展开）

- 教授用自己的话重述了两个任务，顺序与讲义一致：*"we focus on two tasks okay[,] the first task[,] we keep talking about this last week about market return prediction[s]… that's what we learned from last week."*（`08:47`）
- 并预告了后半场的落点——不止预测个股收益，还要**比较股票排名**：*"in the second half of this lecture[,] then we go to the stock return prediction[,] okay[,] individual [stock return prediction,] okay[,] so that's on a weekly basis[,] and then I will construct a lot of technical characteristics for the… firms there[,] and then I will also introduce a couple of the new methods to show you how to… predict [returns] and how to compare stock rankings."*（`09:49`–`10:12`）
- **这段改变了什么**：确认了 p.2 对照表的两个任务名称与顺序；新增了讲义 p.2 没写清楚的一句——"比较股票排名"是本讲后半场真正的落点，呼应 §2.9，不只是"预测个股收益"这么简单。
...
**🎙️ 课堂补充**（`10:23`–`13:14`，约 3 分钟，A · 课上展开）

- 教授明说本讲三讲都靠这一篇论文串起来，并透露了他与作者的师承关系：*"everything for these three classes[,] I think I would link to the same paper[,] okay[,] this is a very influential… paper published in 2020[,] that is my advisor[,] [Bryan is] also in my committee."*（`10:23`）——*"[Bryan] used to be [at] UChicago and then he moved to Yale in 2018… three years[,]… after me… he's in industry now."*（`10:36`，ASR 把 "Bryan" 听成 "brand"，见 §9.5 ASR 样本）
- 明确把 Class 3–5 定位成这篇论文的延伸，并点出 ML 作业与它的关系：*"I want to summarize in this way[:] the lecture[s] we are going to see[,] forecast three[,] class forecast five[,] th[ey're] basically extension[s] for this paper."*（`11:52`）；*"for the final[,] the… assignment you are going to [do]… after the… first half[,] that's also based on this paper… if you… read any paper [after] my course[,] it must be this one[,] and this [paper] is… relatively simple."*（`12:05`–`12:36`）
- **这段改变了什么**：确认了 §2.1.2 的判断——这篇论文不仅是框架来源，也是 ML 投资作业的直接依据；新增了笔记原来没有的背景——教授与 Gu-Kelly-Xiu 三位作者之一 Bryan Kelly 有师生关系，这也是他反复引用这篇论文、认为"相对简单、值得精读"的原因。
...
**🎙️ 课堂补充**（`17:22`–`17:34`，B · 讲了同讲义，无增量）：教授只用一句话带过，确认三个市场统一用美国宏观变量——*"for this example[,] we can use the US macropredictors to predict different markets[,] okay[,] the US one[,] the Asia one[,] and the Hong Kong [one]."*（`17:22`）——没有展开"为什么"或提到港币联系汇率；那个解释要到 §2.5.2（`01:09:55`–`01:10:54`）才出现。和笔记原判断一致：p.5 只是设置，理由在后面。
...
**🎙️ 课堂补充**：⏭️ **这一页（p.6 公式本身）课上没有逐项讲。** 依据：`13:36`–`24:55` 教授把这段时间用在了复习上周内容（`25:09`–`32:18` 点名提问"上周学了什么"）和解释"为什么需要 ML 方法"（`13:50`–`17:22`：相关预测变量、高维数据的定义），随后 `32:18` *"let[’]s introduce the method[s] one by one"* 直接跳到前向选择（p.10，§2.3），全文没有出现对 $L(\alpha,\beta)$ 公式本身的逐项讲解或板书式推导。建议：这一页是 M02 的直接重复，教授默认"上周已经会了"，预习时可以降低优先级，但公式仍要自己会写——它是后面四个模型共同的起点，§6.2 已把它列为 🟡 考点。
...
**🎙️ 课堂补充**（`21:34`–`22:33`，B · 讲了同讲义，无增量）：教授把这一页当"上周复习"带过，没有代入 p.7 的具体数字（0.20%、0.40%、−0.30%），只重申了步骤——*"we usually standardize[,]… we subtract [the] mean[,]… divide by the standard deviation[,] then we obtain… standardized [z]…once you have those[,] the formula[s] here for last month's forecast… you plug in[,] you get your forecast[,] just that easy."*（`21:39`–`22:33`）。核心步骤——"标准化只用训练均值、再代入算预测"——与笔记判断一致，没有新增或推翻的内容。
...
**🎙️ 课堂补充**（`22:33`–`24:55`，约 2.4 分钟，A · 课上展开）

- 教授口头重述了 develop / validate / refit 的结构，并**亲口说出了 178 这个数字**：*"if we have like[,]… 100 [months,]… 300 [months]… this is develop[ment]… this is [the] training sample[,] we estimate the model[,] and then we use the validation sample to choose the model[,] and then once we choose the model there[,] we combine training and validation together[,] we estimate the model with 178 observations there."*（`22:46`）
- 并解释了顺序为什么必须冻结：*"[T]he sequence[:] train[ing,] validation[,] test[,]… must be fixed because we only use past data to estimate the model[,] to select the model[,] and then we use the frozen model to evaluate its performance for future data."*（`23:23`）
- **这段改变了什么**：教授口头给出的 178 与笔记 §2.2.3「refit = 178 个月」完全一致；确认了"先训练、再验证选模型、再合并重估、再冻结测试"的顺序是上一讲就定好的规矩，本讲只是重申，没有新的细节，但这是一处很干净的数字核对。
...
**🎙️ 课堂补充**（`32:30`–`39:12`，约 6.7 分钟，A · 课上展开）

- 教授现场用 p.11 的假数逐步演算了前向选择，与笔记的手算完全一致：*"[T]he first one[,]… we tr[ied] four of them… the one [that] mostly [reduced] validation MSE[,] the first one is [term spread]… we see the MSE reduce from 30 to 28.7… we add a second one[,] the second one turns out to be inflation… 28.7 reduced to 28.2… can we add another one[,] the third one… is unemployment[,] however… it increase[s] from [28.2] to 28.4[,] this means… we should not add this[,]… we should… include [a] two[-]factor model."*（`37:56`–`39:12`）
- 前向选择在教授本科时就是标准方法：*"[T]he method forward step[wise] selection[,]… if you have taken any regression course[,] this might come up in your [curriculum,]… this [came] up in my [curriculum] when I was a sophomore student[,] okay[,] 20 years ago."*（`33:28`）
- 解释了为什么用验证 MSE 而不用训练 MSE：*"[W]hy do we… use the validation sample MSE[,] not the training sample MSE[?] The reason [is] that… the training sample MSE… predictor[s] one by one[,]… always reduce[s]… so those people they call this overfitting."*（`35:05`–`35:17`）
- 并把 MA(12) 一直很稳的原因归结为"它本身没有可调参数"：*"[F]or out of sample prediction[,] usually a smaller model is better than a large model[,]… why… MA 12 is very good[:] those average predictors… don't have any predictors on the right hand side but they are still doing very good because they are [constant] models."*（`36:34`–`36:49`，"CTO models" 疑似 ASR 误听，推断为 "constant models"，标 `[?]`，见 §9.5）
- **这段改变了什么**：教授现场演算与笔记的手算表逐格一致（30.00→28.70→28.20→28.40，停在两变量）；新增了笔记原来没有的两处背景——① 前向选择的"资历"（教授读本科时就学过）；② MA(12) 稳赢的另一种解释——它是一种"约束到底"的常数模型，天然不会过拟合，这比笔记原文"简单模型更稳"更进一步。
...
**🎙️ 课堂补充**（`39:12`–`43:43`，约 4.5 分钟，A · 课上展开）

- 教授引入收缩/正则化的说法，与笔记公式一一对应：*"[A]fter the… selection[,] then I will talk about a more general method[,] they call [it] shrinkage method[,] or they call [it] regularization[,]… usually [they] form… [a] penalty on… [the] regression loss function."*（`39:12`）
- 补了一句笔记里没直接引的第一手动机——很多研究发现 OLS 做样本外预测容易过拟合：*"[M]any researchers find out that if [you] use[] regression to do out of sample forecast[ing,] model[s] usually overfit."*（`39:43`）
- 惩罚的作用：*"[T]hey try to shrink the parameter[,]… to a small number[,] or even… zero[,] so that's the shrinkage method[,] they add a penalty[,] call[ed] P(beta)[,] on those… coefficient[s]."*（`40:44`）
- 两个极端情形，与笔记「极端 / 边界情况」逐句对应：*"[T]here are… two extreme cases[:]… if lambda is zero[,]… we only have the first [term,]… the OLS regression[,]… [if] lambda is… infinity[,]… we [will] shrink all beta hat to almost zero[,] exactly zero[,]… [the forecast is] simply the average."*（`41:49`–`42:51`）
- **这段改变了什么**：教授的讲法与笔记 §2.4.1 的公式、λ 的两个极端完全一致；额外强调的"OLS 做样本外预测通常会过拟合"是收缩方法存在的第一手理由，已补进上文「为什么需要它」。
...
**🎙️ 课堂补充**（`44:47`–`53:29`，约 8.7 分钟，含现场 AI 演示，A · 课上展开）

- 教授引入岭回归时明确给出了它的名字——但整堂课 ASR 几乎每次都把 "ridge" 听成 "which / width / weight"：*"[T]hen we give them the penalty different names[,] okay[,] different names represent for different methods[,] one famous name is called [ridge] regression."*（`44:47`，见 §9.5 ASR 样本）
- 现场用连着 Dropbox 的 AI 工具跑岭回归，指令与讲义 p.25 的流水线是同一件事的口头版：*"[Y]ou need to standardize the predictors[,] okay[,]… and then you fit [the] model with different penalty values[,]… different lambda gives you [a] different alpha hat and beta hat[,]… probably I can ask my AI to do this."*（`46:50`–`47:31`）→ *"My AI is linked with my Dropbox[,]… Can you help me to… [run] ridge regression[,] with penalt[ies] of… 1[, 1,] 10[,]… and print alpha[,] [the] estimate[s]… for all predictors[,] and the intercept[,] as well as the[ir] validation MS[E]."*（`47:31`–`48:36`）
- 读结果：*"[W]e have… the… different lambda penalt[ies] and then we estimate [the] ridge regression[,] this is the validation MSE[,]… you can see that lambda equal[s] [0].1[,] this is the smallest one."*（`49:51`），随后又加了 alpha = 0.01、0.0001 两列对照。
- OOS $R^2$ 公式口头讲解，直接支持 §2.5.1/§2.5.2 的判断：*"[W]hy is this[?] [T]his is the formula for… R squared out of sample[,]… this is [the] ridge forecast[,] the benchmark is [the moving-average] forecast[,]… if the MSE from the ridge model is larger than the MSE from the MA 12[,] then this guy is larger than one[,]… MA[12] is a very strong benchmark."*（`52:37`–`53:11`）
- **这段改变了什么**：教授现场演示确认了 p.14 的核心结论（λ = 0.1 附近验证 MSE 最小，MA(12) 是很强的基准）；"MA(12) is a very strong benchmark" 这句原话直接支持笔记 §2.5.2"除香港外全部输给 MA(12)"的判断，升进 §6.2。
...
**🎙️ 课堂补充**（`53:29`–`58:22`，约 4.9 分钟，含现场 AI 演示，A · 课上展开）

- 引入 LASSO：*"[N]ext[,] another method[,]… the [LASSO] method[,]… if you ask anyone [who] ha[s] taken the machine learning course[,] the data mining course[,] which method they have learned[,] they will tell you [LASSO]."*（`53:29`）
- 解释归零的机制，与笔记「为什么公式长这样」一致：*"[T]he simple change here[,]… previously when you have beta j squared[,]… we penalize the beta[,] but the beta does not equal to… zero exactly[,] they are just small[,]… [with LASSO,] beta[’s] absolute value[,] it is able to get exactly zero[,]… that means [it achieves] [variable] selection[,] if beta j is zero[,] then the [j-th] predictor is out."*（`54:01`–`54:45`）
- 现场演示，把上一段的岭回归表更新成 LASSO 版本：*"Can you update [the] above table with [LASSO]?"*（`54:37`）→ *"[F]or this case you can see that if we compare the OLS… regression coefficient and the [LASSO] coefficient here[,] then two facts[:]… the [LASSO] coefficient is usually smaller than [the] OLS one[,]… the second [fact] is[,] from this table you can see exactly there are two zero[s] here[,] which means that the federal funds rate might not be an important predictor for US market return[,] inflation might not be an important predictor for US [market]."*（`55:49`–`56:36`）
- 把惩罚一路加到很大，验证了「极端情形」：*"[B]ecause here you can see for lambda equal to one[,] non[e] [equal] to 10[,] lambda equal to 100[,] all these numbers they are the same[,]… this [means] this kind of large [penalty]… force[s] all the beta to be zero."*（`57:38`–`57:48`）
- **这段改变了什么**：教授现场把 λ 从 1、10、100 一路调到 0.001，直接验证了「λ 足够大时所有斜率恰好为零」的极端情形；并明确说"这意味着联邦基金利率、通胀在这个惩罚下不重要"——是讲义假数表 p.16 没有的、来自真实美国宏观数据的系数解读。
...
**🎙️ 课堂补充**（`01:03:36`–`01:07:07`，约 3.5 分钟，含现场演示，A · 课上展开）

- 引入弹性网：*"[N]othing special[,] just the combination between [Ridge] and [LASSO] with a combination rule[,]… lambda is a [tuning] parameter[,] rho is a tuning parameter[,] which means [that] for different combinations of lambda [and] rho[,] you get different estimate[s] of alpha and beta."*（`01:03:44`）
- 现场把网格从"三档 λ"扩到"三档 λ × 两档 ρ"：*"[C]an we also… update the table[,] with elastic[,]… different… of four lambdas[,]… probably three choices of lambda[,] and two choices of rho."*（`01:04:07`–`01:04:41`）
- 选模逻辑与告诫：*"[W]e can see that this one provides you the minimum validation MSE[,] okay[,] so we pick this model[,] just that easy[,] then you have your alpha and beta hat estimate[,]… any forecast you form you can just do this[,] this is for statistical implementation[,] it does not mean your forecast makes sense[,] it does not mean your forecast is profitable."*（`01:04:57`–`01:05:19`）
- **这段改变了什么**：教授现场跑出的验证网格与笔记 §2.4.4 引用的假数网格形态一致（选最小格、冻结 (λ, ρ)）；额外补了笔记正文没有的一句警告——"算得出预测不代表预测有意义、不代表能赚钱"，已同步进 §9.2。
...
**🎙️ 课堂补充**（`01:05:19`–`01:06:15`，约 1 分钟，A · 课上展开）

- 教授口头描述了系数路径的两端：*"[T]his is something called regularization path[,]… this is the [curve] for [LASSO][.] [T]his is 10 to the power negative 5[,]… the lambda is almost zero[,]… the numbers here… are almost the same as linear regression[.] … [F]or this part[,] length trend to the [letter] is one[,] some[where]… like infinity[,] so all coefficients they [go] to zero here[.]"*（`01:05:19`–`01:05:54`）
- **归零顺序与笔记逐字对应**：*"[F]rom [the] regularization path[,] you know[,] okay[,] the first one [to] go to zero[,] that's federal fund[s] [rate,] the second one [to] go to zero[,] thumb scrap[,] the third one [to] go to zero is the unemployment [rate,] the last one [to] go to zero is inflation[,] so if you ask me which is most important th[is time]…"*（`01:05:54`–`01:06:15`，"thumb scrap" 是 "term spread" 的 ASR 误听，见 §9.5）——联邦基金利率最先归零（最不重要）、期限利差第二、失业率第三、**通胀最后归零（最重要）**，与笔记「p.19 通胀最晚归零」的判断完全一致。
- **这段改变了什么**：把笔记从"读图猜顺序"升级成"教授口头确认的顺序"，通胀在美国市场任务里最重要这一点从图读推断升为教授明示，可升进 §6.2。
...
**🎙️ 课堂补充**（`01:07:50`–`01:08:32`，A · 课上展开）：教授明确说四个模型的验证损失"可能统计上无法区分"，直接印证了笔记「loss differences are small」的读法——*"[I]f we use all this method[,] OLS[,] [Ridge,] [LASSO,] elastic [net,]… then… [LASSO] will give you the smallest one[,]… but the difference[s]… [are] quite small[,]… if you ask me which method is the best[,] I would just say these three there might be similar[.] I cannot tell [if] these three numbers are statistically [different] because they are very similar."*（`01:07:50`–`01:08:32`）。**这段改变了什么**：把「差别小」从笔记的推断升级为教授原话，已同步进 §6.2 答题依据。
...
**🎙️ 课堂补充**（`05:09`–`06:01` 行政段 + `52:37`–`53:29` 正文，A · 课上展开）

- 教授在开场介绍期末考形式时，**直接把"给数字算样本外 R²"点名为考试题型**：*"[T]o answer [a short-answer] question[,] I only expect no more than 10 words[,] okay[,]… you can see this is the case[,] you will see[,] to calculate out of sample R squared based on the numbers here[,] if you have the formula[,] then it is easy[,] calculation[,]… if I give you simple numbers like this[,] I will not allow any calculator."*（`05:09`–`06:01`）
- 正文里公式本身的口头讲解见 §2.4.2 引用（`52:37`）：*"this is the formula for… R squared out of sample…"*
- **这段改变了什么**：这是**唯一一处教授把某道具体讲义题型直接点名为期末考题型**的地方——"给两个 SSE 算 OOS $R^2$"不只是本讲的计算题，也是任务单问题 2 要找的考试证据，升为 §6.2 🔴。
...
**🎙️ 课堂补充**（`01:08:44`–`01:12:45`，约 4 分钟，A · 课上展开）

- 美国与内地全线告负、香港是唯一赢家：*"[F]or OLS you can see that they underperform the… [MA12] forecast[,] especially for the Asia[,] like this[,] [negative] 148%[.] [O]kay[,] so why do we lose [to] this kind of [lagged MA] forecast[?] [T]he average [was] the best."*（`01:09:26`–`01:09:43`）；*"if you follow [Ridge,]… then 6.1% for Hong Kong…"*（`01:09:43`–`01:09:55`）
- **港币联系汇率——教授明示的经济解释**：*"[I]n the four predictors there [is a] federal fund[s] rate[,] that's also… highly related to the federal funds rate[.] So yesterday you see [the US raise] the… federal fund[s rate] to 3.75 to 4[,] and then this morning you['ll] see the Hong Kong [rates,] they also raise[d] Hong Kong[’s rate] to… 4.25 because there's a peg between the Hong Kong dollar and the US dollar[,]… once the US chang[es its] policy[,] Hong Kong must change."*（`01:09:55`–`01:10:54`）
- LASSO 在香港的解读：*"[T]he lasso[,] you can see[,] okay[,]… US [is] bad[,] Hong Kong one is good[,] the mainland one is like this."*（`01:10:55`）；*"[T]he Hong Kong one is the best for [LASSO because] they [only select] one predict[or,]… inflation there[,] the others they are… zero…"*（`01:12:16`）
- Golden window（MA12）为什么更稳：*"[W]hy usually I would say average might be the best forecast[?] [B]ecause the real life[,] we can [plot] it in this way[,]… if you use the [Ridge/LASSO] forecast[,] it looks [like] this[,] the variation does not match at all[,]… but if you check the 12[-month moving-average window] forecast[,]… it also does not match with the real[ized] return[,] but it is way better than this guy."*（`01:11:20`–`01:11:58`）
- **这段改变了什么**：把笔记原来"美国货币政策对港股影响大是有道理的"这一句 ⚪ 推断，升级为教授明示——港元与美元的联系汇率制度使港元利率必须跟随美元加息，这是"只有香港赢"背后的经济机制，升进 §6.2 🔴；也确认了内地 −148.2% 不是数据错误，只是输给了一个很稳的基准。
...
**🎙️ 课堂补充**（`01:14:37`–`01:16:59`，约 2.4 分钟，A · 课上展开，课堂练习）

- 教授明说这两题就是考试同款：*"I do have two questions for you guys[,] then we take a break[,] okay[.] Multiple [choice] questions[,] this is the same style you will see in the exam."*（`01:14:37`）
- **p.26 当堂确认答案为 A**：学生答题后教授追问 *"Can you explain why you chose[?]… A?"*（`01:15:01`），学生说明理由后教授确认 *"Okay[,] that's correct?"*（`01:15:16`）——与笔记 ⚪ 推断一致，**升为 ✅**。
- **p.27 只讲了标准化的道理，没有明说选项字母**：*"[T]here [are] two different scales there[,] if you want to do any kind of [shrinkage estimator,] you have to standard[ize] the predictor[s] in the same scale[,] this is the first thing you need to do[,]… otherwise you will apply different level[s] of penalty on different beta[s]… so first[,] when you [do] standardization[,] then you go to your… [Ridge] or [LASSO] or elastic [net,]… almost for all the [scikit-learn] package[s]… they will have the standardized step in their package."*（`01:16:18`–`01:16:59`）——内容与选项 D（用 development 样本标准化）一致，逻辑得到教授原话印证，但字母未明说，**维持 ⚪（逻辑已确认）**。
- **这段改变了什么**：任务单问题 1 的三道选择题之一在此解决——p.26 从推断升为课堂确认。
...
**🎙️ 课堂补充**（`01:17:18`–`01:22:04`，约 4.8 分钟，A · 课上展开）

- 切换到个股任务：*"[T]hen we [move on to] individual stock[,] [return] prediction[s][.] There[.] So for what's [next,] you might know that probably you do not know[,] [that the standard deviation] for individual stocks is way higher than market return[s]."*（`01:17:18`–`01:17:31`）
- 现场举了两级噪声对照，笔记表格没有的具体数字：*"[Y]ou can see[,] for other stocks like Apple[,] like Google[,] like Amazon[,] Microsoft[,] Nvidia[,] Meta[,] Tesla[,] this is like two times higher than [the index,] but these are large[-]cap stocks[.] [A]ctually there are a lot of… small[-]cap stocks[.] … [W]hich variation… can be… a few times [higher] than the index."*（`01:21:38`–`01:22:04`）
- **这段改变了什么**：确认了 §2.6.1 表格"单只股票噪声极大"这一行的量级——大盘股（苹果、谷歌等）波动约为指数两倍，小盘股数倍于指数，是讲义原表没有的具体例子。
...
**🎙️ 课堂补充**（`01:18:33`–`01:20:04`，约 1.5 分钟，A · 课上展开）

- 教授现场展示的正是这份固定股票池的表格：*"[T]his is the data set for July [2023]…you can see that this is [from the] index[,]… we have about 80 stocks[,] okay[,] in 2023[.] [S]o you can see like Xiaomi[,] like [Tencent]… all those stocks[,] you know[,] by the names[,] are here."*（`01:18:56`–`01:19:34`）
- 并读出了具体列名：*"[T]his is the stock return for that week[,]… and then this is the Hang Seng [I]ndex return for that week[,]… and then this is the US treasury yield[,] the [three-]month treasury yield[,] for that year."*（`01:19:47`）
- **这段改变了什么**：确认了 §2.6.2/§2.6.5 的字段设计（个股周收益、恒指周收益、美国 3 个月国债收益率）就是这份真实表格的列；额外给出了小米（Xiaomi）作为股票池成员的具体例子，讲义原表没有列出个股名字。
...
**🎙️ 课堂补充**（`01:28:40`–`01:30:53`，约 2.2 分钟，A · 课上展开）

- 教授承认自己也叫不出全部六族的名字，呼应笔记"记问题、不记名字"的策略：*"[F]or a particular [feature,] we kind of have like six categor[ies],… categories for predictor[,] for trend[,] for activity[,] for volatility[,] for market [risk],… if you ask me what are they[?] [S]omething[,] I cannot answer all of them."*（`01:28:54`–`01:29:08`）
- **Citadel 轶事**：*"I used to work [at] Citadel when I was a PhD student[.]… For different teams[,]… they use different labels[.] [S]o for this team they call [it] X1[,] for the other team[,] they try to hide secrets from others or try to avoid overlap[ping] with the same signal there[,] so they use different labels[,]… even for the staff they use different ID[s] too."*（`01:29:20`–`01:29:44`）
- 学术金融 vs 业界量化的立场对比：*"[I]n [the West,] they do not care about your model[.] They care about the story[.] They care about whether the investment logic is solid[.]… [I]f you keep telling that you don't care about what predictor[s] is[,] you only care about… X1, X2[,] they will [think you're] crazy[.]… [F]or academic finance[,] like me[,]… I do care about th[e] interpretation."*（`01:30:00`–`01:30:53`）
- **原始数据源就是 Yahoo Finance 的日频 OHLCV**（教授自己标为 important）：*"[T]his is one important thing I want to tell you. So this is the data we download from [Yahoo] Finance.… [T]he opening price, the high price, the low price,… the close price, the volume.… [T]hen we use the data… to construct our stock characteristics."*（`01:30:53`–`01:31:14`）——三十列技术指标没有任何外部数据，全部由这五列日频数据算出来，这也是为什么 `class03/` 只需要一张价格表就能复算。
- **这段改变了什么**：新增了讲义没有的一段行业背景——量化机构常把技术指标匿名成 X1/X2 以防泄密、不同团队甚至用不同的员工 ID 体系；而教授代表的学术金融立场更看重"故事"和经济逻辑。价值高，已同步进 §9.2。
...
**🎙️ 课堂补充**（`01:31:14`–`01:39:14`，约 8 分钟，含历史视频片段，A · 课上展开）

- 为什么用排序而不是像宏观变量那样做 z 分数：*"[P]reviously[,] when I tell you we need to standardize like inflation[,]… we subtract the mean[,] we divide it by the standard deviation[.] But in this case[,] we are doing ranking[.] Why [are] we doing ranking[?]… [T]his is [the wisdom] from… the traditional investment industry[.]… [computer science or engineering,] we do not care about this[,] but we appreciate the academic wisdom for this."*（`01:31:14`–`01:32:14`）
- 排序方法：*"[I]f we have like 80 stocks in the [Hang Seng] index[,]… then you rank from 0 to 1[,] one is the top[,] 0 is the lowest[,]… then you will have the ranking of this[,] like 80 stock[s]."*（`01:35:56`–`01:36:13`）
- **直接确认"减 0.5"这一步**：*"[S]ometimes they want to [shift] it to like negative 0.5 to 0.5[,] so they subtract everything by 0.5[,] so 0 [is] in the middle."*（`01:38:49`）——与笔记 §2.6.4「中心化排名 = 百分位 − 0.5」逐字对应。
- **整个下半场的总论点**（教授原话 "the whole message"）：*"The whole message I want to give you [today]… [ranking] stock is important. Ranking stock has a long tradition in the investment industry[,]… many, many of the hedge funds[,] they are doing this for a long time."*（`01:45:21`–`01:45:38`）——为什么本讲个股部分从头到尾都在讲"排序"而不是"预测精度"：这是业界几十年的做法，机器学习只是换了产生排序的工具（→ §2.9.2）。
- 历史渊源：教授放了自己多年前录的一段介绍因子投资的视频，并推荐了 Fama 的诺贝尔演讲：*"[T]his is the video[,]… we call it… multiple years ago when I was young[.]… [I]f you have time[,] I would highly suggest you [watch the] Nobel Prize [lecture from] Fama[.] [T]here's a video for that."*（`01:32:14`–`01:40:12`）
- **这段改变了什么**：**这一段直接实证了 §2.6.4 的核心公式**——上引 `01:38:49` 那句"减 0.5、让 0 在中间"与讲义 p.32 逐字对应；额外补充了排序传统的学术渊源（Fama 1992/93 因子研究），让笔记「为什么用排名」的论证有了教授原声支持，不只是讲义书面说法。
...
**🎙️ 课堂补充**（`01:19:47`–`01:20:48` + `01:46:01`–`01:46:31`，A · 课上展开）

- 换算：*"[Y]ou can see that this is weekly return[,] we need to divide [this] by [52] weeks[,] so you get 0.00[08]1 or something[.] And then [the] excess[-]return… [is] this guy subtract this guy[,]… you get this guy."*（`01:20:04`–`01:20:14`）
- 教授在此还顺带区分了"相对无风险的超额收益"与"相对大盘的主动收益"两个概念：*"[T]he risk[-]free way adjusted… [return,] and then the so[-]called active return[,]… [is] the individual stock return subtract the market return."*（`01:20:14`–`01:20:37`）——笔记只定义了前者，后者（active return，主动收益）是笔记没写的姊妹概念，本讲后续只用超额收益。
- 再次确认换算方式：*"[F]or the left hand side[,] for individual [stock returns,]… we use the [excess] return[s]…we use the short[-]term treasury yield and then divide it by [52] weeks[,]… because we are using weekly [return data] now."*（`01:46:01`–`01:46:31`）
- **这段改变了什么**：确认了 §2.6.5"周收益要除以 52 周"的换算方法；新增了笔记没写的相关概念"主动收益（active return）= 个股收益 − 大盘收益"，提醒读者本讲不用它，避免与超额收益混淆。
...
**🎙️ 课堂补充**（`01:48:10`–`01:55:31`，约 7.4 分钟，A · 课上展开）

- 动量五分组：*"[T]his is the momentum quint[ile,]… this is 100[-]day momentum[.]… we… rank[,] and then we split them into five groups[.] Group one, group two, group three and good [four,] good five[,] is the [highest momentum]… good [one]… is the lowest."*（`01:50:10`–`01:50:39`）；*"[I]f those stocks [performed] well last week, they keep performing well[.]… if those stocks… perform[ed] bad last week[,] they keep performing bad last week."*（`01:51:14`–`01:51:25`）
- **波动率反直觉——教授用词与笔记完全一致**：*"[F]or the… realized volatility[,]… if the volatility is high[,] the… return is… low[.] The volatility [is the] highest group[,] the… return of the… lowest group [is]… high[.] This is something I would… say[:] counterintuitive."*（`01:51:36`–`01:51:59`）
- 价差量级：*"[Y]ou have a spread for about 5%[,]… a[bout] 0.4%[.] [I]f you long this guy[,] you short this guy[,] you [could] [pocket] 0.5%… that's 35%[.] [T]his is a very huge spread… for [an] annualized… return there."*（`01:53:26`–`01:53:54`）
- **前视偏差告诫**：*"[M]ost signals[,] most characteristics[,] most factors [that were] discover[ed],… expose[d]…this [is called] look[-]ahead bias[.]… [D]on't be too surprised[,]… like[,] if other people tell you they can get profit… every year for the past 10 years[,] this is something like [an] expos[ed] finding[.] Can you still use this strategy for [the] next year? We don't know."*（`01:54:11`–`01:55:31`）
- **这段改变了什么**：教授明确用 "counterintuitive" 形容波动率分组的反常现象——这是笔记用词的教授原声依据，升为 §6.2 🔴；新增了笔记没有的重要告诫——已发表因子普遍有前视偏差，历史表现好不保证未来有效，已同步进 §6.2 🔴 与 §9.2。
...
**🎙️ 课堂补充**（`01:55:43`–`01:56:55`，A · 课上展开）：教授明确说出了自己对"线性 vs 机器学习"的立场——*"[I]f we go back to our best friend[,] regression[,]… we assume a single regression can be used to predict all stocks[.] Previously[,] we only use one regression to predict the market[,] and now we have like 80 stock[s]…we have like 156 weeks[,]… more than 12,000 observations there[,] so we [have] one line of regression with many observations."*（`01:55:43`–`01:56:20`）——*"[E]ven though you see the title[,] machine learning[,] there[,] I still believe that the best strateg[y] is [still] regression."*（`01:56:28`–`01:56:42`）。**这段改变了什么**：确认了 §2.6.7 的"一条规则共用于所有股票"设定；教授这句价值判断（哪怕标题写机器学习，最好的策略仍是回归）本身也是很好的答题素材，已同步进 §9.2。
...
**🎙️ 课堂补充**（`01:57:24`–`02:00:11`，约 2.8 分钟，A · 课上展开，课堂问答）

- 教授反复追问："Can you tell me why B is wrong?"（`01:58:00`、`01:58:36`）
- 学生部分作答：*"[I]t's also very close to the minimum[.] So the support[ive] [evidence]… difference."*（`01:57:51`）
- 教授总结，核心逻辑与笔记 C 选项一致：*"I can only say you will get some partial credit[] for this[,] because for this sentence[,]… I just want to tell you that [the] validation MSE[s] are very[,] very close[.] Okay[,] 0.0102 [/] 0.0100 [/] 0.0101[.] Okay[,] they're very[,] very close[.] And then we [call] the statistical[]indifferen[ce] between these three numbers[,]… of course they cannot dominate statistically[,] they are the same thing[,] from a statistical sense."*（`01:59:42`–`02:00:11`）
- **这段改变了什么**：任务单问题 1 里第三道选择题（p.38）的课堂原始记录——教授没有直接念出正确选项字母，而是引导学生认识到"三个验证 MSE 统计上不可区分"，与笔记判断 C 的理由完全一致，并明确给出"部分学分"的评分方式；因为没有听到字母本身，**维持 ⚪（逻辑已明示，字母未明示）**。
...
**🎙️ 课堂补充**（`02:00:34`–`02:02:50`，约 2.3 分钟，A · 课上展开）

- 教授的口头描述与笔记判断一致：*"[N]ow we move to the method[,] dimension reduction[.] So for the [first predictors] there[,] for the 30 categories there[,] one thing we see [is] that they are highly[,] highly correlated[.]… this is the heat map for the [correlation][.]… black means… one."*（`02:00:34`–`02:00:46`）
- 信号可能远少于 30 个：*"[E]ven though you have high[-]dimensional predict[ors,] say 30 of them,… but the actual signal they carry might be less than 10[,] less than 15[,] less than 20[.]… [C]an we transform them to something [uncorrelated]?… [P]rinciple component method is a method [for] how to transform them."*（`02:01:01`–`02:01:31`）
- **这段改变了什么**：确认了 §2.7.1"30 个特征高度相关、真实信号可能不到 20 个"的判断；没有给出精确的相关系数或累计方差数字（0.25、61% 等仍只能靠讲义渲染图目测，见 §9.5）。
...
**🎙️ 课堂补充**（`02:02:50`–`02:09:37`，约 6.8 分钟，A · 课上展开）

- 定义与算法：*"[H]ow [do] we cho[o]se the component[?]… V[i] is your characteristics[.] You have about 30… standardized characteristics[.]… you are going to create a combination for th[ese] characteristics[.]… you want to maximize… the varian[ce] for a linear combination[.]"*（`02:03:18`–`02:04:12`）
- 正交约束：*"[B]ecause… the purpose we see here[,] we do not want [] correlated predictors[.] We want something uncorrelated[.] So we want to create a second component[,]… [orthogonal to] the previous one."*（`02:04:37`–`02:04:47`）
- **p.43 载荷图的口头解读**：*"[I]f you want to interpret the component[s] there[,] you can tell others the first PC[,] they load on the high 52[-week gap,] on the downside… residual volatility for 100 days[,]… low on the RSI 20 days[,]… the[y] load on th[ese] things[.]… For the second component[,] they load on… the… four characteristics [related to] volatility[,]… they load on this characteristic to the beta market 60 days."*（`02:09:06`–`02:09:37`）
- **这段改变了什么**：教授现场口头解读了 p.43 的载荷图——第一主成分正向载荷在"52 周高点差、20 日 RSI"这类趋势/振荡器特征上，负向载荷在"100 日下行已实现波动率"这类波动特征上；第二主成分主要载荷在"60 日市场 beta"——与笔记引用的讲义原文方向一致，且给出了具体特征名对应关系，直接回应了任务单问题 7。
...
- ❌ "载荷图给每个主成分都能配一个清楚的经济名字" → 教授讲完 p.43 的载荷解读后自己加了限定：*"[This] might be the most important kind of perspective to explain each component. Okay. But of course the interpretation might not be that clean… from time to time. But you can still say something."*（`02:09:46`–`02:09:56`）——能说"PC1 偏趋势、PC2 偏市场 beta"，但别指望每个成分都有干净的标签。
...
**🎙️ 课堂补充**（`02:09:56`–`02:11:28`，约 1.5 分钟，A · 课上展开）

- 引入 PLS：*"[T]he second method for dimension reduction is called partial [least squares][.] So previously[,]… for principal component analysis[,] we only use information on the right hand side[.] We haven't touched the [variation] from the left hand side[.] But this one [uses] both sides[.]… [T]hey construct a linear combination… to maximize the covariance between this component and the return."*（`02:09:56`–`02:10:24`）
- 教授本人的取舍判断：*"[I]f your goal is for return prediction[,] I would say [PLS] is better[.]… But PLS[,] the drawback of PLS[,] [is]… there's no analytical form[.]… [W]ith PC[A,] the mathematical properties… people can develop tons of research based on [PCA][.] PLS[,]… they are [fewer][.] But if you ask me which [prediction] is better[,] I'd say PLS is better [for] prediction."*（`02:11:05`–`02:11:28`）
- **这段改变了什么**：教授明确给出了"预测选 PLS、数学性质选 PCA"这一取舍判断，是他本人的偏好陈述，直接印证了笔记「为什么需要它」的论证，已同步进 §9.2。
...
**数字读法**（`code/L03_linear_ml/01_reproduce_lecture.py` B 段，用 `stock_linear_test_predictions.csv` 的 4,108 行重算；2026-09-22 用重下的完整文件复核，下面各条成立，见 §9.5 #14）
...
**🎙️ 课堂补充**（`02:12:06`–`02:12:52`，A · 课上展开）：教授口头确认了六模型比较表的结论——*"[N]ow let's see the performance for all these methods[.]… [F]or the validation MSE[,] you can see the numbers[,] they are very close[.]… [I]f you ask me which one is [best,] all of them are the same[.]… [B]ut out of sample R squared[,] this guy[,] these two guys are positive[.]… [I]t seems that the component method[s] also work[ed] quite well[.]… [F]or all others[,] there [is] much [less]… relative[.] Except for this guy[,] OLS[.]… it's bad[,] OLS."*（`02:12:06`–`02:12:52`）。**这段改变了什么**：确认了验证 MSE 几乎相同、只有 PCR/PLS 测试 OOS $R^2$ 为正、OLS 最差这一结论；没有提到 GKX 论文里线性模型在美股上的量级作对照。
...
**🎙️ 课堂补充**（`02:16:05`–`02:16:48`，B · 讲了同讲义，无增量）：教授确认了这张表的核心教学点——预测集中在 −1% 到 +1%，实现值波动极大，方向有时也会错：*"[T]his is a table to show you for Hang Seng [I]ndex stock[s]… CCB is the kind of construction bank[.]… you can see for one thing that our forecast[s,] they kind of… small numbers[,] okay[,] between… negative 1% to 1%[,] the small numbers there[.] But the realized return[s,] they have huge variation[.]… so this is something you already expect[:] the forecast[s] from all these [models,] they cannot tell you… much[.]… crazy[.]… so sometimes… the number can be far away from… and even [the] sign can [be wrong] for this one."*（`02:16:05`–`02:16:48`）。转录里**只明确点名了建设银行（CCB）**，没有听到教授逐一念出美团、信义光能的名字或数字——笔记这三行来自讲义原表，转录只确认了整体读法，不算新增数字。
...
**🎙️ 课堂补充**：❓ **录音范围内没有听到教授专门朗读或演示这条"比较六个模型 + 追踪一只股票"的 AI 指令。** 前后对照：`02:11:28`（PLS 取舍讨论结束）到 `02:12:06`（"now let's see the performance for all these methods"）之间没有间隔，教授像是直接展示已经算好的结果表（p.47），没有像 p.25 那样现场读指令、跑代码、读结果；也没有出现"追踪一只指定股票"的现场操作（p.49 的三只股票像是直接引用讲义原表，见 §2.8.2）。**不能标成 ⏭️ 略过**——本讲 scan 显示 0 处 ≥120 秒空档，更可能是"确实没有出声演示"而非"漏录"。不可降权：这一页的方法论（指令怎么写、为什么要"trace the named stock"）只能靠讲义与本笔记学。
...
**🎙️ 课堂补充**（`02:17:41`–`02:19:00`，约 1.3 分钟，A · 课上展开）

- **ML 投资作业剧透，直接焊在这一页上**：*"I can promise you this is the assignment for your homework[.] You will see this very[,] very soon[.] Okay[.] So for example[,] one thing that is[,] we have those [shrinkage] methods there[,] is the penalty… too high[,] because you know the signal is kind of like very[,] very small."*（`02:17:41`–`02:17:57`）
- 解释常数预测怎么发生：*"[I]f you use like BIC[,] [forward] selection[,]… or [cross-validation] selection[,] those different selection criteria and selection methods there[,] they might tend to choose a big penalty[.] Once you have a big penalty[,]… all the beta[s]… force[d] to be zero[,]… all… you [have left] is… alpha hat[.] So what do I mean by using alpha hat[,] to predict the left hand side? This means that you use a constant number to predict all stocks[,] in all the way[.] Okay, this is crazy, but this is the case there."*（`02:18:03`–`02:18:40`）
- 警告：*"[I]f you use this kind of [shrinkage] method[,] I highly encourage you to… not use those [huge] penalt[ies] there[.] Otherwise you will use a single number to predict everything[.] This is crazy."*（`02:18:48`–`02:19:00`）
- **这段改变了什么**：教授把"作业剧透"直接和常数预测这一页焊在一起——ML 投资作业会让学生自己改训练/验证/测试窗口长度、重新跑排序（呼应 `02:17:21` *"if you change the window length… you might see totally different numbers there"*），而"惩罚过大导致常数预测"正是作业里最容易踩的坑，教授用"crazy"这个词强调了两次。已升进 §6.2 🔴 与 §9.2（作业提示），回应任务单问题 3。
...
**🎙️ 课堂补充**（`02:13:40`–`02:15:16` + `02:22:36`–`02:23:12`，A · 课上展开）

- 教授给这套方法起了名字，与笔记「预测分组排序」的中英对照一致：*"[F]or each method[,] we can calculate r hat[.] We can [sort by it][.]… if you [rank] stock[s] based on… r hat[.]… [I]f you predict this is high[-]ranking stocks for high return[,] then they should have high returns[.] If this is low[-]ranking stock[s]…, they should have low returns[.]… [F]or all different methods there[,] it seems we have some monotonic trend[.]… [S]o this is called forecast sort[folio,] or forecast[-]impl[ied] long[-short] portfolio."*（`02:13:58`–`02:14:59`）
- 口头确认了两端价差的数字：*"[T]he spread… the top[,] the top stock is like 0.20%[,] the bottom stock is like negative[ly] 0.28%[.] If you create a long[-short] portfolio[,] that's 0.48%[.]… you can expect to… [annualize]… 25%."*（`02:22:48`–`02:23:12`）
- **这段改变了什么**：教授口头把"按预测分组"这套方法称为 "forecast sort portfolio"，与笔记译名"预测分组排序"完全对应；并确认了五组的两端价差（+0.20%/−0.28%/合计 0.48%）与讲义 p.55 逐格一致。
...
**🎙️ 课堂补充**（`01:59:20`–`02:00:34` + `02:20:07`–`02:20:45`，A · 课上展开）：教授现场举的例子与讲义 p.54 的三行例子是同一个论点的不同数字版本——*"[S]o which one will the choice[?]… [S]o one thing I want to emphasize here[,] is[:] two criteria[,] they might deliver two different result[s,] or two different conclusions[.]… [I]f this is the real[ized return,] for [a] stock[,] if you have method one[,] you have method two[.] You can see that the forecast is simply twice of the [other][.] And this is like four times[.] So what do you see[?] If you calculate R squared[,] model A has a high[er] R squared than model B[,] because it[s] return forecast is closer to this[,]… [b]ut if you calculate ranking[,] if you long this stock and short this stock[,]… the stock ranking is the same for model A and model B."*（`02:20:07`–`02:20:45`）。**这段改变了什么**：确认了"MSE 看水平、排序不受单调变换影响"这一结论不止讲义写了，教授在课上也专门用另一组数字（两倍/四倍）重讲了一遍，说明这是他反复强调的重点。
...
**🎙️ 课堂补充**（`02:19:09`–`02:19:36` + `02:22:36`–`02:23:38`，A · 课上展开）

- 统计证据弱、经济证据扎实：*"[T]he statistical evidence is extremely small[,] extremely close to zero[.] But economic evidence based on stock ranking is very solid[.] It seems that we have the story[.]… [T]he return forecast[s] order[/]generate profits for you[.] It seems like that way."*（`02:19:09`–`02:19:36`）
- 用整讲的收尾句总结个股部分：*"[T]he spread… the top stock is like 0.20%[,] the bottom stock is like negative[ly] 0.28%[.] If you create a long[-short] portfolio[,] that's 0.48%[.]… the ranking… tell[s] you that the long[-short] portfolio[,] the top minus bottom[,] they have a really huge number for [the] spread[,] on… 48%… every week."*（`02:22:48`–`02:23:12`）→ *"[F]or example[,] like this figure[,] if you want to [sort them] into five portfolio[s,] the ranking might require… growth[.] So that's… the economic evidence I want to convince you."*（`02:23:26`–`02:23:38`）
- **这段改变了什么**：教授用"我想说服你们的经济学证据"这句话为整个排序论证收尾，明确把"统计上几乎为零、经济上很扎实"定为本讲个股部分最重要的结论，与笔记「所以呢」的判断完全一致。**没有**听到教授对 t = 1.46 或前后半段差异给出具体口头评价——这两个数字更像是讲义视觉复核得到的，§9.5 待核对表保留这一条。
...
**🎙️ 课堂补充**：❓ **录音范围内没有讲到留给非线性的问题。** 转录在 `02:24:04` 中断——*"That's why we [use] dimension reduction[.] Okay[.] Because we are talking about… [big data]."*——总结刚开始就被切断（`02:23:49` *"let me summarize with the last slide"*），没有听到"阈值""交互"这两个词，也没有 Class 4 预告的具体内容（会讲哪些模型）。**这不能标成 ⏭️ 略过**——p.56 是最后一页，总结明显讲到一半被截断（见文首提示块「缺结尾」），不是教授翻页跳过。本节只能靠讲义与本笔记学；M03 当堂 Class 4 具体预告仍未录到；M04 开头提供了部分后续证据。

**↪️ M04 开头的后续补证（不等于恢复 M03 原话）**：M04 源 A `04:00`–`06:47` 回顾了高维变量与弱样本外信号，A `11:27`–`13:23` 说明线性规则难表达不同状态，并把非线性作为新的动机；对应 [[M04-非线性机器学习与收益预测|M04]] §2.1。它确认了 p.56 的“同一问题换非线性”方向，**没有**补出 M03 当堂对宏观收缩效果、个股排序行为的完整总结。因此本页仍保留 ❓ 原录音缺结尾，不能标为完整课堂讲解。
...
| 级别 | 含义 | 本讲数量 |
|---|---|---|
| 🔴 教授明示 | 转录里教授明确说过会考 / 要记 / 规定了答题方式，或直接给出经济解释 —— 引用原话 | **7 条**（本讲转录 `M03-transcript.txt`） |
...
| ⚪ 笔记推断 | 根据内容重要性、课堂逻辑已确认但字母未明说、或 M02 🔴 考点的延续 | 5 条（p.27 / p.38 选项逻辑已由课堂问答确认，字母维持 ⚪） |

> M02 的 6 条 🔴（含期末题型与及格线，见 [[M02-回归与样本外设计#6.2 考点清单|M02 §6.2]]）里"期末是手写、有计算题、要会读表"三条在本讲转录里再次被印证（`02:53`–`06:43` 行政段）。

### 6.2 考点清单

| 可信度 | 考点 | 依据 | 对应小节 |
|---|---|---|---|
| 🔴 | **给两个 SSE 算样本外 $R^2$，是期末考原题型**（教授明说不许用计算器、会给简单数字） | 🎙️ `05:35`–`06:01`：*"you will see to calculate out of sample R squared based on the numbers here… if I give you simple numbers like this I will not allow any [calculator]"* | §2.5.1 |
| 🔴 | **只有香港赢过 MA(12)，原因是港币联系汇率**：美国加息至 4.0%，香港次日跟涨至 4.25% | 🎙️ `01:09:55`–`01:10:54`：*"there's a peg between the Hong Kong dollar and the US dollar… once the US chang[es its] policy[,] Hong Kong must change"* | §2.5.2 |
| 🔴 | **波动率分组"高波动、低收益"是教授明确定性的反常结论** | 🎙️ `01:51:47`–`01:51:59`：*"This is something I would… say[:] counterintuitive."* | §2.6.6 |
| 🔴 | **已发表因子普遍有"前视偏差"，历史赚钱不保证未来有效** | 🎙️ `01:54:11`–`01:55:31`：*"this [is called] look[-]ahead bias… Can you still use this strategy for [the] next year? We don't know."* | §2.6.6 |
| 🔴 | **惩罚过大 → 常数预测，是 ML 投资作业最容易踩的坑（教授两次用 "crazy" 强调）** | 🎙️ `02:17:41`–`02:19:00`：*"I can promise you this is the assignment for your homework… Otherwise you will use a single number to predict everything. This is crazy."* | §2.8.4 |
| 🔴 | **p.26 选择题答案当堂确认为 A**（验证选完就冻结、refit、测试只评一次） | 🎙️ `01:15:01`–`01:15:16`：*"Can you explain why you chose… A?"* … *"Okay[,] that's correct?"* | §2.5.4 |
| 🔴 | **系数归零顺序：联邦基金利率最先归零，通胀最后归零（最重要）** | 🎙️ `01:05:54`–`01:06:15`：*"the last one [to] go to zero is inflation"* | §2.4.5 |
...
> 页码 = `Lec03_Linear_Machine_Learning.pdf` 的页码（右下角 n/56）。「类型」列标出封面页与章节分隔页（整页只有一个章节名）；「课堂覆盖」列对照 `M03-transcript.txt`（`00:56 → 02:24:04`，587 段）：✅=课上讲了（详讲/简讲）、🎙️=🔴 或高价值补充、⏭️=有证据翻过去、❓=录音未覆盖、⚪=封面/分隔页。

| 讲义页 | 类型 | 内容 | 笔记小节 | 课堂覆盖 |
|---|---|---|---|---|
| p.1 | **封面页** | 封面：Class 3: Linear Machine Learning for Return Prediction | — | ⚪ 封面 |
| p.2 | 内容 | 两个预测任务的目标与基准 | §2.1.1 | ✅ 简讲 `08:47`–`10:12` |
| p.3 | **分隔页** | 章节分隔页：Market Prediction with Linear ML | §2.1 开头 | ⚪ 分隔页 |
| p.4 | 内容 | Gu, Kelly & Xiu (2020) 与 Class 3–5 路线 | §2.1.2 | 🎙️ 详讲 `10:23`–`13:14` |
| p.5 | 内容 | 四个美国宏观变量与滞后 | §2.1.3 | ✅ 简讲 `17:22`–`17:34` |
| p.6 | 内容 | OLS 目标函数与预测式 | §2.2.1 | ⏭️ **未讲**（见 §9.1） |
| p.7 | 内容 | OLS 手算下月预测（假数） | §2.2.2 | ✅ 简讲 `21:34`–`22:33`（作为上周复习） |
| p.8 | 内容 | 冻结时间线 142 / 36 / 60；OLS 澄清 | §2.2.3 | ✅ 详讲 `22:33`–`24:55`（口头确认 178） |
| p.9 | **分隔页** | 章节分隔页：Selection and Shrinkage | §2.3 开头 | ⚪ 分隔页 |
| p.10 | 内容 | 前向选择的定义与更新规则 | §2.3 | ✅ 详讲 `32:30`–`39:12` |
| p.11 | 内容 | 前向选择手算表（假数） | §2.3 | ✅ 详讲（同上，现场逐格核对） |
| p.12 | 内容 | 收缩的一般形式：损失 + λ·惩罚 | §2.4.1 | ✅ 详讲 `39:12`–`43:43` |
| p.13 | 内容 | Ridge 定义、用途、效果 | §2.4.2 | 🎙️ 详讲 + AI 演示 `44:47`–`53:29` |
| p.14 | 内容 | Ridge 三步流程；美国 alpha = 100、验证 27.02、OOS −3.2%；software alpha = nλ | §2.4.2 | 🎙️ 详讲（同上，现场跑出结果） |
| p.15 | 内容 | LASSO 定义、角、选择告诫 | §2.4.3 | 🎙️ 详讲 + AI 演示 `53:29`–`58:22` |
| p.16 | 内容 | LASSO 读系数与预测（假数） | §2.4.3 | 🎙️ 详讲（同上） |
| p.17 | 内容 | Elastic Net 定义、λ 与 ρ | §2.4.4 | ✅ 详讲 + AI 演示 `01:03:36`–`01:07:07` |
| p.18 | 内容 | Elastic Net 验证网格（假数） | §2.4.4 | ✅ 详讲（同上） |
| p.19 | 内容（图） | LASSO 系数路径（美国） | §2.4.5 | 🔴 简讲，归零顺序口头确认 `01:05:19`–`01:06:15` |
| p.20 | 内容（图） | Ridge / LASSO 验证曲线 | §2.4.5 | ✅ 简讲（同上区间） |
| p.21 | 内容 | 美国四模型验证 MSE 表 | §2.4.6 | ✅ 简讲 `01:07:50`–`01:08:32` |
| p.22 | 内容 | OOS $R^2$ 计算题 | §2.5.1 | 🔴 行政段明示为考题型 `05:35`–`06:01`；公式讲解 `52:21`–`53:29` |
| p.23 | 内容 | 三市场测试 OOS $R^2$ 表；不许事后换模型 | §2.5.2 | 🔴 详讲，港币联系汇率 `01:08:44`–`01:11:20` |
| p.24 | 内容（图） | 四幅图：滚动收益、OOS 柱状、美国预测 vs 实现、选中系数 | §2.5.2 | ✅ 详讲（同上，golden window 图解读） |
| p.25 | 内容（代码） | Codex 拟合 Ridge / LASSO 的流水线 | §2.5.3 | ✅ 现场演示融入 p.13–18（见 §2.4.2–§2.4.4 🎙️ 格） |
| p.26 | 内容 | 选择题：验证后下一步 | §2.5.4 | 🔴 课堂练习，答案 A 当堂确认 `01:14:37`–`01:15:16` |
| p.27 | 内容 | 选择题：单位与标准化 | §2.5.4 | ✅ 简讲（逻辑确认，字母未明说）`01:16:18`–`01:16:59` |
| p.28 | **分隔页** | 章节分隔页：Stock Returns and Characteristics | §2.6 开头 | ⚪ 分隔页 |
| p.29 | 内容 | 市场问题 vs 个股问题对照表 | §2.6.1 | ✅ 简讲 `01:17:18`–`01:22:04` |
| p.30 | 内容 | 固定 2023-07 股票池 | §2.6.2 | ✅ 详讲 `01:18:33`–`01:20:04` |
| p.31 | 内容 | 六族技术指标 | §2.6.3 | 🎙️ 详讲（Citadel 轶事）`01:28:40`–`01:30:53` |
| p.32 | 内容 | 日频 → 周度预测行；中心化排名；无泄漏 | §2.6.4 | 🔴 详讲（"减 0.5"口头确认）`01:31:14`–`01:39:14` |
| p.33 | 内容 | 超额收益与无风险代理 | §2.6.5 | ✅ 简讲 `01:19:47`–`01:20:48` |
| p.34 | 内容 | 零超额收益算例；个股 OOS $R^2$ 公式 | §2.6.5 | ✅ 简讲（同上，另见 `01:46:01`–`01:46:31`） |
| p.35 | 内容（图） | 一周截面的 30 个排名热图 | §2.6.6 | ❓ 转录未见专门讲解这张热图（讨论排序时只带过一般概念） |
| p.36 | 内容（图） | 特征分组组合收益（四个特征） | §2.6.6 | 🔴 详讲（"counterintuitive"）`01:48:10`–`01:55:31` |
| p.37 | 内容 | 截面线性规则；周度切分表；恒生银行缺失 | §2.6.7 | ✅ 简讲（"best strategy is still regression"）`01:55:43`–`01:56:55` |
| p.38 | 内容 | 选择题：哪个线性工具匹配业务目标 | §2.6.8 | ✅ 课堂问答 `01:57:24`–`02:00:11` |
| p.39 | **分隔页** | 章节分隔页：Dimension Reduction | §2.7 开头 | ⚪ 分隔页 |
| p.40 | 内容（图） | 秩相关热图 + 累计方差曲线 | §2.7.1 | ✅ 简讲 `02:00:34`–`02:02:50` |
| p.41 | 内容 | PCR 定义 | §2.7.2 | ✅ 详讲 `02:02:50`–`02:04:47` |
| p.42 | 内容 | PCR 预测算例（假数） | §2.7.2 | ✅ 简讲（未见现场重算假数） |
| p.43 | 内容（图） | PCR 两个成分的载荷 | §2.7.2 | 🎙️ 详讲，载荷口头解读 `02:09:06`–`02:09:37` |
| p.44 | 内容 | PLS 定义 | §2.7.3 | ✅ 简讲 `02:09:56`–`02:10:24` |
| p.45 | 内容 | PLS 预测算例（假数） | §2.7.3 | ✅ 简讲（PCR/PLS 取舍 `02:11:05`–`02:11:28`） |
| p.46 | **分隔页** | 章节分隔页：Weekly Prediction Evidence | §2.8 开头 | ⚪ 分隔页 |
| p.47 | 内容 | 六模型比较表（真数） | §2.8.1 | ✅ 详讲 `02:12:06`–`02:12:52` |
| p.48 | 内容（图） | 四幅图：验证损失、OOS 柱状、斜率热图、分组收益 | §2.8.1 | ✅ 简讲（同上，四图未逐一口述） |
| p.49 | 内容 | 一周三只股票 | §2.8.2 | ✅ 简讲（仅点名 CCB）`02:16:05`–`02:16:48` |
| p.50 | 内容 | Vibe coding 指令 | §2.8.3 | ❓ **录音没覆盖**（见 §9.1） |
| p.51 | 内容 | 常数预测不能排序 | §2.8.4 | 🔴 详讲，ML 作业剧透 `02:17:41`–`02:19:00` |
| p.52 | **分隔页** | 章节分隔页：Prediction Accuracy and Stock Rankings | §2.9 开头 | ⚪ 分隔页 |
| p.53 | 内容 | 按预测分组的四步 | §2.9.1 | ✅ 详讲 `02:13:40`–`02:15:16`、`02:22:36`–`02:23:12` |
| p.54 | 内容 | 量级 vs 排名三行例子 | §2.9.2 | ✅ 详讲 `02:20:07`–`02:20:45` |
| p.55 | 内容 | Ridge 分组价差表 | §2.9.3 | 🔴 详讲，收尾定论 `02:19:09`–`02:23:38` |
| p.56 | 内容 | 留给非线性的问题 | §2.9.4 | ❓ **M03 原录音缺结尾**；M04 A `04:00`–`13:23` 部分回顾方向，未恢复 p.56 完整当堂总结 |
...
**课堂覆盖统计**（49 页内容页）：✅ 讲了 **45 页**（含 🔴/🎙️ 标注的 12 页）｜ ⏭️ 略过 **1 页**（p.6）｜ ❓ 未录到 **3 页**（p.35、p.50、p.56）。另有约 15–20 分钟纯课堂内容讲义无对应页（行政事务 `00:56`–`08:32`、职业闲聊 `58:22`–`01:03:36`、AI 编码建议 `01:12:45`–`01:14:37`），已提取到 §9.2 与相关元文件。

**课堂时间分配**（总录音 `00:56 → 02:24:04`，约 143 分钟，按时间戳）：

| 内容块 | 转录区间 | 用时 | 占比 |
|---|---|---|---|
| 行政事务（TA/期末考/评分/分组，讲义无对应页） | `00:56`–`08:32` | ~7.6 min | 5.3% |
| 市场任务框架回顾 + GKX 论文介绍（p.2, 4） | `08:32`–`13:26` | ~4.9 min | 3.4% |
| 为什么需要 ML（高维数据动机） | `13:26`–`25:09` | ~11.7 min | 8.2% |
| 上周内容点名提问 | `25:09`–`32:18` | ~7.2 min | 5.0% |
| 前向选择（p.10–11） | `32:18`–`39:12` | ~6.9 min | 4.8% |
| 收缩总论（p.12） | `39:12`–`44:47` | ~5.6 min | 3.9% |
| Ridge + AI 现场演示（p.13–14） | `44:47`–`53:29` | ~8.7 min | 6.1% |
| LASSO + AI 演示（p.15–16） | `53:29`–`58:22` | ~4.9 min | 3.4% |
| AI 感想 / 职业与人生闲聊（讲义无对应页） | `58:22`–`01:03:36` | ~5.2 min | 3.6% |
| Elastic Net + 系数路径 + 验证选模（p.17–21） | `01:03:36`–`01:08:32` | ~5.0 min | 3.5% |
| 市场任务测试结果三市场（p.22–24） | `01:08:32`–`01:12:45` | ~4.2 min | 2.9% |
| AI 编码建议（讲义无对应页） | `01:12:45`–`01:14:37` | ~1.9 min | 1.3% |
| 课堂选择题 + 休息（p.26–27） | `01:14:37`–`01:17:18` | ~2.7 min | 1.9% |
| 个股任务：数据与六族指标（p.29–31） | `01:17:18`–`01:31:14` | ~13.9 min | 9.7% |
| 排序方法论 + Fama 视频 + 因子历史（p.32） | `01:31:14`–`01:48:10` | ~16.9 min | 11.8% |
| 排序图证据：动量、波动率反直觉、价差（p.36） | `01:48:10`–`01:55:43` | ~7.5 min | 5.2% |
| 截面回归公式 + 选择题（p.37–38） | `01:55:43`–`02:00:34` | ~4.9 min | 3.4% |
| 降维：相关热图 + PCR（p.40–43） | `02:00:34`–`02:09:56` | ~9.4 min | 6.6% |
| PLS（p.44–45） | `02:09:56`–`02:11:52` | ~2.0 min | 1.4% |
| 个股结果表 + 三只股票例子（p.47, 49） | `02:11:52`–`02:17:41` | ~5.8 min | 4.0% |
| 常数预测 + ML 作业剧透（p.51） | `02:17:41`–`02:19:09` | ~1.5 min | 1.0% |
| 排序证据 + 价差解读（p.53–55） | `02:19:09`–`02:23:38` | ~4.5 min | 3.1% |
| 总结（未完，p.56） | `02:23:38`–`02:24:04` | ~0.4 min | 0.3% |

> 用时按内容块**分界时间戳的差值**估算，块内含穿插的学生互动；百分比之和因四舍五入约为 100%。
...
| 讲义页 | 内容 | 判定 | 依据 | 建议 |
|---|---|---|---|---|
| p.6 | OLS 目标函数 $L(\alpha,\beta)$ 的逐项讲解 | ⏭️ 未讲 | `13:36`–`24:55` 教授把这段时间用于复习上周内容（`25:09`–`32:18` 点名提问）与解释"为什么需要 ML"（`13:50`–`17:22`），`32:18` *"let's introduce the method one by one"* 直接跳到 p.10 前向选择，全文未见对公式本身的逐项讲解 | 这是 M02 的直接复习，预习时可降优先级；但公式仍要自己会写，是后面四个模型共同的起点（§6.2 🟡） |
| p.35 | 一周截面的 30 个排名热图 | ❓ 未见专门讲解 | 教授讨论排序方法时（`01:31:14`–`01:39:14`）只给出一般性说明，没有专门对着这张热图逐行解释；不排除展示 slide 时被泛化进了排序方法的讲解里 | 读图方法（横向看一只股票、纵向看一个特征）仍按讲义自学，不因"没有专门讲解"而降低优先级 |
| p.50 | Vibe coding：Codex 比较六个模型（个股任务） | ❓ 录音没覆盖 | `02:11:28`（PLS 讨论结束）直接跳到 `02:12:06` 展示 p.47 结果表，未闻教授现场读指令或演示；scan 显示本讲 0 处 ≥120 秒空档，更可能是没有出声演示而非漏录 | 不可降权：指令怎么写、为什么要"trace the named stock"做一致性检验，只能靠讲义与本笔记学 |
| p.56 | 留给非线性的问题（Class 4 预告） | ❓ M03 原录音缺结尾，M04 仅部分补证 | M03 `02:24:04` 中断；M04 A `04:00`–`06:47` 回顾高维与简单基准，A `11:27`–`13:23` 引出非线性，但未逐句补回 M03 的总结 | 不可降权：方向已获后续课堂确认；宏观收缩与个股排序的 p.56 原始口头总结仍只能靠讲义与笔记 |

### 9.2 课上讲了但课件没有（🎙️ 最高价值清单）

| # | 内容 | 时长 | 时间戳 | 小节 | 为什么值钱 |
|---|---|---|---|---|---|
| 1 | 🔴 **行政段全内容**：TA tutorial（周日上午 Zoom）、期末考形式（A4 手写 cheat sheet、2 小时、选择题同款、mock exam）、评分构成、随机分组 | ~7.6 min | `00:56`–`08:32` | 已回写 [[EF5560_Fintech_and_AI_in_Finance/_meta/作业与DDL\|作业与DDL]] / [[EF5560_Fintech_and_AI_in_Finance/_meta/考点库\|考点库]] | 讲义完全没有这一段，却直接决定期末考怎么考、参与分怎么算 |
| 2 | 🔴 **港币联系汇率解释"为什么只有香港赢"**：美国加息至 4.0%，港元次日跟涨至 4.25% | ~1 min | `01:09:55`–`01:10:54` | §2.5.2 | 把笔记原来的 ⚪ 推断升级为教授明示的经济机制 |
| 3 | 🔴 **Ridge / LASSO / Elastic Net 现场 AI 演示全过程**（连 Dropbox 的 AI 工具、调 λ/ρ、读结果表） | ~22 min | `44:47`–`01:07:07` | §2.4.2–§2.4.4 | 讲义 p.25 只给了一段代码，课堂演示补上了"怎么问 AI、怎么读它给的表"的完整过程 |
| 4 | 🔴 **ML 投资作业剧透**：改训练/验证/测试窗口长度重跑排序，警惕惩罚过大导致常数预测 | ~1.3 min | `02:17:41`–`02:19:00` | §2.8.4 | 唯一一处教授明确谈到作业内容，直接点出最容易踩的坑 |
| 5 | 🔴 **波动率分组"反直觉"（counterintuitive）+ 前视偏差告诫** | ~1.5 min | `01:51:36`–`01:55:31` | §2.6.6 | 教授明确定性 + 给出已发表因子普遍衰减的警告 |
| 6 | **Citadel 轶事**：量化机构把特征匿名成 X1/X2 防泄密，学术 vs 业界的立场对比 | ~1.6 min | `01:29:20`–`01:30:53` | §2.6.3 | 讲义没有的行业背景，帮助理解"六族"命名法为什么重要 |
| 7 | **GKX 论文与导师 Bryan Kelly 的师承关系** | ~2.9 min | `10:23`–`13:14` | §2.1.2 | 解释了教授为什么反复引用这篇论文、认定它"相对简单值得精读" |
| 8 | **p.38 课堂问答**：三个验证 MSE 统计不可区分 + 部分学分评分方式 | ~2.6 min | `01:57:24`–`02:00:11` | §2.6.8 | 课堂原始问答记录，比讲义题面更有说服力 |
| 9 | **p.43 载荷图口头解读**：趋势/振荡器类正载荷，波动/风险类负载荷 | ~0.5 min | `02:09:06`–`02:09:37` | §2.7.2 | 讲义只给图，教授把 PC1/PC2 对应到具体特征名 |
| 10 | **系数归零顺序确认**：联邦基金利率最先归零，通胀最后归零（最重要） | ~0.3 min | `01:05:54`–`01:06:15` | §2.4.5 | 把"读图猜顺序"升级为教授明示 |
| 11 | **PCR vs PLS 取舍判断**：预测选 PLS、数学性质选 PCA | ~0.7 min | `02:11:05`–`02:11:28` | §2.7.3 | 教授本人的偏好陈述，直接支持笔记论证 |
| 12 | **"哪怕标题是机器学习，最好的策略仍是回归"** | ~0.1 min | `01:56:28`–`01:56:42` | §2.6.7 | 教授对本讲结论的价值判断，适合作答题素材 |

> 没有进入这张表但已在相应 🎙️ 格记录的：Xiaomi 等具体股票池成员名（§2.6.2）、"主动收益"这一姊妹概念（§2.6.5）、Fama 因子研究历史视频（§2.6.4）。
...
本次课同时更新了 syllabus：Class 5 主题由 "From forecasts to portfolios" 改为 **"Interpretable machine learning and portfolio decisions"**；ILO 2 加了 "including the limits of coefficients, feature importance, and response profiles"；course purpose 加了 "compare linear and nonlinear models, interpret model outputs"。这与本讲 p.4 的路线（Class 5 = characteristic importance + portfolio performance）一致。考核、日期、参与规则未变。已回写 [[EF5560_Fintech_and_AI_in_Finance/_prep/课程前置资料|课程前置资料]] 与 [[EF5560_Fintech_and_AI_in_Finance/00-课程总览|00-课程总览]]。

### 9.5 待核对

| # | 事项 | 说明 |
|---|---|---|
| 1 | ✅ **本讲转录已合并**（2026-09-17） | `M03-transcript.txt`：`00:56 → 02:24:04`，587 段，2h23m08s，无时序倒退，0 处 ≥120 秒空档 |
| 2 | ✅ **空档均已查明** | 本讲 scan 显示 0 处 ≥120 秒空档，不需逐个核查空档性质 |
| 3 | ⚠️ **完整性判定：缺开头 + 缺结尾** —— ⚪ 推断 | 依据：(a) 首段 `00:56` 已是回答学生关于 syllabus 的提问，判断缺开头约 1 分钟；(b) `02:23:49` 总结刚开始、`02:24:04` 中断，无下课语、无作业布置，判断缺结尾 |
| 4 | ✅ **`shared/` 四个文件已随 class04 数据包到位**（2026-09-21，见 [[M04-非线性机器学习与收益预测]] §1 与数据集卡片 §13；`code/L04_nonlinear/02_refit_trees.py` 已用它们重训模型）。原记录：| `class03/README.md` 引用 `../shared/market_excess_return_panel.csv`、`hsi_stock_excess_return_panel.csv`、`technical_characteristics_dictionary.csv`、`risk_free_yields_monthly.csv`，导出包与转录里都没有出现这四个文件；`01:18:56` 教授只说数据在 Dropbox / Canvas，没有点名 `shared/`。仍需向教授 / TA 索要；p.35 / p.40 / p.43 精确数字暂无法本地复现 |
| 5 | 选择题与计算题答案 | p.22 = **−20%**（✅ 公式计算，且已用真实 Ridge 数据交叉验证）；p.26 = **A**（✅ 课堂当堂确认，`01:15:01`–`01:15:16`）；p.27 = D（⚪ 逻辑经课堂问答印证，字母未明说）；p.38 = C（⚪ 逻辑经课堂问答印证，字母未明说，`01:57:24`–`02:00:11`） |
| 6 | ⚪ p.19–20、p.24、p.35、p.40、p.48 的读数 | 由渲染图目测，精确值仍需 `shared/` 数据重算；p.43 载荷方向已获教授口头确认（§9.2 #9） |
| 7 | ✅ 所有真实数字已复核（2026-09-22 用重下的完整文件再次确认） | p.14、21、23 的市场数字与 `market_linear_*` 两表逐格一致；p.47、49、55 的个股数字用重下的 `stock_linear_test_predictions.csv`（4,108 行）重算：六个 OOS $R^2$ −0.531 / −0.151 / −0.136 / −0.136 / +0.024 / +0.054% 逐一对上，预测标准差 0.22%–0.41%，LASSO / EN 取值个数 = 1——§2.8.1 / §2.9.3 原文成立。2026-09-21 曾因本地文件被同步截断为 358 行而标为"部分复核不成立"，已撤销 |
| 8 | ✅ 讲义假数算例已复核 | p.7、11、16、18、22、34、42、45、54（`code/L03_linear_ml/01_reproduce_lecture.py` A 段） |
| 9 | 〔?〕p.14 "software alpha = nλ" 里的 n | 转录没有涉及这个换算细节；仍按 178 算了一次示意（§2.4.2） |
| 10 | 〔?〕恒生银行为什么没有数据 | 转录同样没有提到；可能是 2025 年被汇丰私有化要约后停牌 / 退市，待核（🔗 未查证） |
| 11 | 〔?〕p.49 "kind of construction bank" 的股票代码 | 转录只听到 "CCB"，没有给出代码；按讲义原表 0939.HK 处理 |
| 12 | 〔?〕"QP fuel"、"funnel three and three" | 转录里两处听不清的片段（`01:18:03`、`01:16:18` 附近），语境不足以还原，按 `[?]` 处理，不猜 |
| 14 | ✅ **`stock_linear_test_predictions.csv` 曾被同步截断，2026-09-22 已用重下的 `class03.zip` 恢复** | 2026-09-21 发现本地只有 98,250 B / 358 行（manifest 1,127,308 B / 4,108 行）。根因不是脚本：Remotely Save 经代理上传大文件中途停滞，dufs 把半截文件直接写在目标路径，下一轮同步按"取较新"把半截拉回本地覆盖（根因见根 README 待办「同步截断事故根因」）。重下的包是 Canvas 更新版（15 个文件，含 `shared/` 五个面板与系数 / 验证 / 五分组表），sha256 全部一致；`L03_01` / `02` 重跑通过，#7 已复核 |
| 13 | ✅ ASR 校正记录：`01:46:17` "32 weeks" → `[52]` | 转录原文是 "divide it by 32 weeks"，按上下文（周频数据、年化国债收益率换算成周）应为 52，笔记引文已加 `[ ]` 标注；独立验收 2026-09-18 查出并补记 |

**2026-09-26 后续补证**：M04 双录音源 A `04:00`–`13:23` 回顾了线性方法的限制及非线性动机；已回填 §2.9.4、§8、§9.1。它没有补回 M03 p.56 当堂原话的全部细节，故不更改本讲“缺结尾”判定。

**本讲新增的 ASR 错误样本**（已同步追加到 `.claude/skills/transcript-merge/reference/asr-dictionary.md`）

| 转录原文 | 应为 | 首见 |
|---|---|---|
| which regression / width regression / weight regression | ridge regression | `44:47` |
| brand | Bryan [Kelly]（教授导师，Gu-Kelly-Xiu 论文作者之一） | `10:23` |
| thumb scrap | term spread | `01:05:54` |
| federal fund way / federal fund[s] way | federal funds rate | `18:12` 起多次 |
| unemployment wave | unemployment rate | `01:05:54` |
| CTO models | [?]（推断为 constant models，未确认） | `36:49` |
| camera（"upload to the camera"） | Canvas | `01:18:56` |
| QP fuel | [?]（未还原，语境为恒指相关指数名） | `01:18:03` |

**反方视角（对抗自检第 12 项，2026-09-17）**

1. **最薄弱的一节：§2.7.1 与 §2.6.6 的部分读数（p.35 热图）。** p.35 未见课堂专门讲解，`shared/` 数据仍然缺失无法重算精确相关系数；若考试要求精确数字，本笔记给不出。
2. **如果考了这一讲，我现在答不上的：** ① p.27 / p.38 选择题的正确选项字母（逻辑已确认，字母教授没有明说）；② 任何模型的具体系数数值表（讲义与转录都只给图或部分示例）；③ 30 个技术指标的精确定义（词典文件仍然缺失）；④ Class 4 具体讲哪些非线性模型（录音在总结前中断）。
3. **我做的推断，以及错了会怎样：** ① p.27 = D、p.38 = C（⚪，逻辑已获课堂问答印证）——若字母错了，选项分析仍成立，只需换字母；② software alpha 的 n 取 178（⚪）——只影响 §2.4.2 一句示意换算；③ "CTO models" 推断为 "constant models"（`[?]`）——若错，§2.3 一句类比要删，不影响前向选择本身的结论；④ p.35/p.40/p.43 的目测读数——若偏差超过 0.05，对应句子要改，但结论（单调、相关性高、趋势正波动负）不受影响。
...
| 2026-09-17 | **合并 W3 转录**（`M03-transcript.txt`，`00:56 → 02:24:04`，587 段）→ v1.0：35 个 🎙️ 格全部回填（A 29 / B 3 / C 1 / D 2 + 4 页标 ⏭️/❓：p.6 ⏭️，p.35/p.50/p.56 ❓），§6 增 7 条 🔴（含 p.26 课堂确认、港币联系汇率、常数预测=作业剧透、波动率反直觉、前视偏差、OOS $R^2$ 考题型、系数归零顺序），§8 加课堂覆盖列与课堂时间分配表，§9.1/9.2/9.5 按转录重写，frontmatter 改 `transcript: merged`/`status: v1.0`；无 W2 顺延内容需回填 |
| 2026-09-22 | **撤销勘误**：`class03.zip` 从 Canvas 重下（更新版 15 个文件，sha256 全部一致），`stock_linear_test_predictions.csv` 恢复 4,108 行，`L03_01` / `02` 重跑，§2.8.1 与 §2.9.3 的个股数字全部复核成立；§9.5 #7 恢复 ✅、#14 改为已解决并记根因（同步截断，非脚本） |
| 2026-09-21 | **勘误**：`class03/stock_linear_test_predictions.csv` 被发现为截断文件（358 / 4,108 行，9/17 19:48 同步事故），§2.8.1 数字读法与 §9.5 #7 改为"部分复核不成立"，新增 #14；#4 `shared/` 四个面板已随 class04 到位（M04 用它们重训）。正文其余未动 |
| 2026-09-18 | **独立验收后修补**（sonnet 只读验收：15 条抽样引文无实质错误、四道题课堂答案核实、期末考形式块逐条相符、零基础 5/5）：`01:46:17` 引文 "32 weeks" 改为 `[52]` 并在 §9.5 #13 记录；4 处改写补 `[ ]`（28.2 / variable / long-short / PLS）；补 3 处教授标 important 但未引用的原话——§2.6.3 数据源 Yahoo Finance OHLCV（`01:30:53`）、§2.6.4 "the whole message… ranking stock is important"（`01:45:21`）、§2.7.2 载荷解读不总干净（`02:09:46`）；考点库 36/38 补定级依据、"回溯偏差"统一为"前视偏差"；作业与DDL 及格人数改为转录原话 3 人 |

| 2026-09-26 | **M04 开头后续补证**：源 A `04:00`–`13:23` 部分回顾 M03 的高维和非线性动机，回填 §2.9.4、§8、§9.1、§9.5；M03 p.56 原始课堂结尾仍缺，保持 ❓。 |
...
- 元数据：[[EF5560_Fintech_and_AI_in_Finance/_meta/知识层级台账|知识层级台账]] ｜ [[EF5560_Fintech_and_AI_in_Finance/_meta/术语表|术语表]] ｜ [[EF5560_Fintech_and_AI_in_Finance/_meta/考点库|考点库]] ｜ [[EF5560_Fintech_and_AI_in_Finance/_meta/作业与DDL|作业与DDL]] ｜ [[EF5560_Fintech_and_AI_in_Finance/_meta/数据集卡片|数据集卡片]]
- 课程背景：[[EF5560_Fintech_and_AI_in_Finance/_prep/课程前置资料|课程前置资料]] ｜ [[EF5560_Fintech_and_AI_in_Finance/00-课程总览|00-课程总览]]
