...
1. **图里是否用了指定变量与单位？** 应逐图核对 `UNRATE` 是失业率百分比、`DGS10` / `DFF` 是以百分比表示的年化利率、`CPIAUCSL` 是价格指数水平；若画的是课程派生的 `inflation_yoy`，纵轴必须改写“CPI 同比 %”，不能冒称它是 `CPIAUCSL` 指数。只看曲线形状不能证明列和单位用对了，需把图例与数据列名、来源元数据对上。
...
**四问的示例判断（讲义 p.49；以本讲现有图为证，不把“图上没看到”写成统计检验）**：① 价格图先查 2006-07 至 2026-06 是否覆盖目标 240 个月，再回原始价格和公司行动核对复权；曲线平滑不等于复权正确。② 收益图上 CSI 300 的 2008 年危机月明显极端（§2.9.2），应查是否真实事件，并逐月检查缺口 / 可疑零值，而非直接删点。③ 滚动波动率应在危机期抬高、平静期回落；若用来预测下月，窗口必须截止于预测日之前。④ 预测变量与下月收益的散点云很散（§2.9.3），肉眼看不出强关系；不能因为分箱后线略上升就断言样本外可预测。
...
**选第 3 题做一份可复算的参考解**（⚪ 笔记演练；讲义说三选一，未给固定股票 / 指标答案）：把“截至上周的 4 周动量”定义为 $x_{t-1}=P_{t-1}^{\mathrm{adj}}/P_{t-5}^{\mathrm{adj}}-1$，目标是第 $t$ 周实现收益。预测 **2026-06-26** 时，只能用 **2026-06-19** 的已知复权收盘价 $298.01$ 和四周前 **2026-05-22** 的 $308.82$；代入得 $298.01/308.82-1=\mathbf{-3.500\%}$。**2026-06-26 后来实现的 −4.775% 只用于事后核对，不参与算预测变量**（`code/L01_data/check_exercises.py` 从课程 AAPL CSV 复算）。这一步只完成“造变量并验证时点”，不能从一行就声称该变量能预测收益；要评价它，还需用冻结的时间切分与基准做样本外检验。若选第 1 / 2 题，必须先指定两只股票或新增哪条 FRED 序列，题面没有这些具体对象，不能编造唯一数值结果。
...
**把四问用于本库数据的参考作答**（⚪ 笔记核查；课堂现场输出未保存，不能冒充教授当堂答案）：① `spy_monthly_features_240m.csv` 一行是一个**预测月**，滞后特征来自该月之前，`market_return` 是该月实现的目标；必须在代码里逐列核对窗口，不只看列名。② AAPL 文件提供 `adjusted_close`，可用来计算跨拆股 / 派息可比的收益；但仅凭这个列名还**不能证明**数据商每次公司行动都处理正确，需与原始收盘价和公司行动记录抽样对照。③ `macro_240m.csv` 的月份标签是参考期，不是发布日；文件不含逐条实际发布日期或财报归档日，故**不能凭这张表证明所有宏观 / 会计值当时可用**，应补发布日历 / 原始归档日再核。④ “能否从干净环境重跑”需要在新环境实际安装依赖、运行代码并比对输出行数、日期、图和关键数字；存在脚本并不等于已完成这个独立验证，课堂现场产物缺失时结论应写“尚未验证”。
