...
**ML Investment Project 正式截止：2026-10-15（周四）14:00（Canvas 当前页面显示；截图未显示时区）。** 交付内容见 [[EF5560_Fintech_and_AI_in_Finance/_meta/作业与DDL|作业与DDL]] §2.2。
...
| 4 | Nonlinear machine learning for return prediction | [[M04-非线性机器学习与收益预测]] | ✅ **v1.0**（2026-09-26）：54 页讲义全覆盖，A/B 同课双录音互补，37 个 🎙️ 格回填；课堂实际到 p.39，p.40–54 ↪️ 下讲顺延 |
...
> **转录**：M01–M03 已合并；M04 用 `transcripts/M04-transcript-A-partial.txt` 与 `M04-transcript-B-partial.txt` 两份同课重叠录音互补。A 补 B 的 boosting／三题，B 补 A 的 bagging；各源时间戳独立。详见 [[M04-非线性机器学习与收益预测]] §8–9。
...
2. ~~确认 ML 投资作业截止时刻~~ ✅ Canvas 当前显示 **2026-10-15 14:00**；正式发布日期、作业详情和提交入口仍到 Canvas 作业页核对（见 [[EF5560_Fintech_and_AI_in_Finance/_meta/作业与DDL|作业与DDL]] §4）。
